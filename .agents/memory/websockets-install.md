...
**AD1 fix (TickResult frozen=True):** `TickResult` was `@dataclass` (plain). Changed to `@dataclass(frozen=True)`. The 4 thermal fields written post-construction in `_drive()` section B converted to `dataclasses.replace()` (`_dc_replace` alias). frozen=True makes thread-safety structural rather than reasoning-based — any future mutation attempt raises `FrozenInstanceError` immediately. Note: `checkpoint_states: dict[str,str]` field is still a mutable dict; frozen only prevents rebinding the reference, not mutations of the dict contents — but the control plane never mutates it after construction.

**AD2 fix (bounded advisory executor):** `asyncio.to_thread()` has no executor parameter. Changed to `asyncio.get_running_loop().run_in_executor(_ADVISORY_EXECUTOR, functools.partial(...))`. Module-level `_ADVISORY_EXECUTOR = ThreadPoolExecutor(max_workers=4, thread_name_prefix="advisory")` in `run_manager.py`. 5 runs × 6 agents = 30 concurrent tick-1 LLM calls queued safely behind 4 workers. Load test 1× with agents enabled + bounded executor: **27.746 s [PASS]** (was 26.303 s with default pool — small delta from queueing, still well under 30 s NFR).
