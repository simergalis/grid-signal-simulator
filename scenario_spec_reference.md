...
| `intrinsic_output_ramp_rate_mw_per_s` | float > 0 \| null | `null` | Always-on intrinsic stack real-power ramp limit for each synchronized, producing block (MW/s), provenance `proposed`. Omitted derives `effective_block_rated_mw / (3 × 3 s)`, using the approximate 95% settling point of the three-second fuel-to-power lag: full scale in about 9 s, or about 667% of rating/min. This engineering placeholder is deliberately below the manufacturer's published 2,000%/min extreme-case system figure, which includes supercapacitor assistance while the stack recovers and is not the stack's intrinsic capability. No manufacturer-published step-load response time in seconds exists for this equipment; the block default and aggregate model's 60%/min rate therefore bracket an unvalidated response. It applies with or without `fuel_system`; pressure, delivery, and utilisation constraints may reduce output further but cannot bypass this limit. Synchronization establishes the minimum-stable floor after start dwell; stop and protection trips disconnect to zero rather than operating below that floor. The standard 5 s reporting cadence can hide the ramp trajectory; use 1 s results for ramp-model comparisons. |
