...
export interface TickPayload {
  ...
  p_imbalance_mw:       number         // generation − demand (positive=surplus, negative=deficit)
  ...
  capacity_gate_deferred_count: number  // Kube jobs deferred by MW-cap gate this tick (0 when inactive)
  capacity_gate_headroom_mw:    number  // design_peak_load_mw − p_compute_demand_mw (negative = over cap)
...
export interface KubeClusterMetrics {
  cluster_id: string
  tenant_id: string
  scheduler_type: 'SLURM' | 'K8S' | 'RAY'
  hardware_profile_id: string
  capacity_unit: 'node' | 'rack'
  gpus_per_unit: number
  max_units: number
...
export interface KubeMetrics {
  ...
  cluster_metrics?: KubeClusterMetrics[]
...
export interface KubeClusterSpec extends KubeJobSpec {
  cluster_id: string
  tenant_id: string
  scheduler_type: 'SLURM' | 'K8S' | 'RAY'
  capacity_unit: 'node' | 'rack'
...
export interface ScenarioSpec {
  ...
  kube_clusters?: KubeClusterSpec[] // heterogeneous independent scheduler clusters
