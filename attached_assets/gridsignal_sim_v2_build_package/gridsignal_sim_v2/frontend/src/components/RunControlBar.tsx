...
export function RunControlBar({ runId, lastRunId, isPaused, onRunStarted, onRunStopped, onRunPaused, onRunResumed, onNewScenario, onViewResults }: Props) {
  ...
  // GPU Generator — when running, its config is wired into the backend kube scheduler
  const genRunning = useGpuGeneratorStore(s => s.running)
  const genConfig  = useGpuGeneratorStore(s => s.config)
  ...
  // Auto-restart the run when the GPU Generator is started mid-run.
  // The generator_config_override is only included in POST /runs when genRunning
  // is true at the moment the request fires.  If the user starts the generator
  // while a run is already in progress, we stop the current run and immediately
  // start a fresh one so the backend KubeDemandAgents are created and kube_metrics
  // flows into the tick stream.
  const prevGenRunningRef    = useRef(false)
  const restartInFlightRef   = useRef(false)
  useEffect(() => {
    const wasRunning = prevGenRunningRef.current
    prevGenRunningRef.current = genRunning
    if (restartInFlightRef.current) return
    if (!wasRunning && genRunning && runId && selectedId) {
      restartInFlightRef.current = true
      ;(async () => {
        setBusy(true)
        setError(null)
        try {
          // 1. Cancel the active run
          const stopResp = await fetch(`/runs/${runId}`, { method: 'DELETE' })
          if (!stopResp.ok && stopResp.status !== 404) {
            const text = await stopResp.text()
            throw new Error(`DELETE /runs/${runId} → ${stopResp.status}: ${text}`)
          }
          onRunStopped()
          // 2. Restart — genRunning is now true so generator_config_override is included
          const startResp = await fetch('/runs', {
            method: 'POST',
            ...
            body: JSON.stringify({
              ...
              generator_config_override: genConfig,
            }),
          })
          if (!startResp.ok) {
            const text = await startResp.text()
            throw new Error(`POST /runs → ${startResp.status}: ${text}`)
          }
          const data = await startResp.json() as { run_id: string; soc_floor_pct?: number; soc_ceil_pct?: number }
  ...
  const handleStart = async () => {
    ...
    const startScenarioId = selectedId
    setBusy(true)
    setError(null)
    try {
      const resp = await fetch('/runs', {
        ...
        body: JSON.stringify({
          ...
          ...(bessRatedMw   !== null && { bess_rated_mw_override:   bessRatedMw }),
          ...(bessUsableMwh !== null && { bess_usable_mwh_override: bessUsableMwh }),
          // When the GPU Generator is running, wire it into the backend kube
          // scheduler so that Slurm / K8s / Ray job arrivals drive real compute
          // MW in the simulation.  The backend translates GeneratorConfig →
          // kube_config before building the run context.
          ...(genRunning && { generator_config_override: genConfig }),
        }),
      })
      if (!resp.ok) {
      ...
      // AND the operator hasn't manually started the generator already.
      // When genRunning is true the operator's config was already wired into the
      // backend via generator_config_override — don't overwrite it with the preset.
      //
      // Task #339: don't trust the component's `selectedSpec` here. It is
      // populated by an async, fire-and-forget scenarioStore fetch that can
      // still be in flight (right after page load, or immediately after
      // switching scenarios) when the operator clicks Start — in that window
      // `selectedSpec` is null/stale and the preset would silently never arm.
      // Fetch the spec for the scenario the run actually started with, fresh,
      // so the auto-arm decision never depends on component render timing.
      if (!genRunning) {
        try {
          const specResp = await fetch(`/scenarios/${startScenarioId}`)
          if (specResp.ok) {
            const detail = await specResp.json() as { spec?: { generator_config?: unknown } }
            const generatorConfig = detail.spec?.generator_config
            if (generatorConfig) {
              ...
              gen.restartWith(generatorConfig as Parameters<typeof gen.restartWith>[0])
            }
          }
        } catch {
          // Non-critical — operator can still arm the generator manually from
          // the Compute Racks modal.
        }
      }
    } catch (e) {
      setError(String(e))
