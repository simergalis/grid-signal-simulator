...
/** Keep only GPU load steps that can be reached during the configured run. */
export function trimGpuLoadProfileForDuration(
  points: [number, number][],
  durationSeconds: number,
): [number, number][] {
  return points.filter(([timeSeconds]) => timeSeconds <= durationSeconds)
}
...
const HARDWARE_PROFILES = [
  ...
  { id: 'nextgen_rack_liquid', label: 'nextgen_rack_liquid — liquid-cooled (120 kW/rack)' },
]
...
export function ScenarioBuilder({ editId, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/scenarios/${editId}`)
      ...
      .then(d => {
        ...
        // Older JSON-backed scenarios can omit a profile or carry an empty
        // irradiance array. The editor always needs a visible first irradiance
        // row; an empty GPU profile explicitly means a flat 100% load.
        setSpec({
          ...loaded,
          irradiance_steps: loaded.irradiance_steps?.length > 0
            ? loaded.irradiance_steps
            : [[0, 1]],
          gpu_load_profile: loaded.gpu_load_profile ?? [],
          pms_config: (loaded as ScenarioSpec & { pms_config?: typeof loaded.pms_config }).pms_config ?? null,
        })
  ...
  const setGpuLoadProfile = (pts: [number, number][]) => {
    const inWindow = trimGpuLoadProfileForDuration(pts, spec.end_sim_time)
    patch({ gpu_load_profile: inWindow })
  }

  const setEndSimTime = (endSimTime: number) => {
    const inWindow = trimGpuLoadProfileForDuration(spec.gpu_load_profile ?? [], endSimTime)
    patch({
      end_sim_time: endSimTime,
      gpu_load_profile: inWindow,
    })
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const inWindowGpuLoadProfile = trimGpuLoadProfileForDuration(
        spec.gpu_load_profile ?? [],
        spec.end_sim_time,
      )
      const specWithPhysics: ScenarioSpec = {
        ...spec,
        // Do not persist steps after the run window. This also protects older
        // scenarios loaded with a profile that predates duration validation.
        gpu_load_profile: inWindowGpuLoadProfile,
