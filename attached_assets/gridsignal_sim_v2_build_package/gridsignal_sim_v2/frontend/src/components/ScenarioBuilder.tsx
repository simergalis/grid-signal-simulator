...
export function ScenarioBuilder({ editId, onClose, onSaved }: Props) {
  ...
  const setGpuLoadProfile = (pts: [number, number][]) => {
    ...
    patch({ gpu_load_profile: inWindow.length > 0 ? inWindow : undefined })
  }
  ...
  const setEndSimTime = (endSimTime: number) => {
    ...
    patch({
      ...
      gpu_load_profile: inWindow.length > 0 ? inWindow : undefined,
    })
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const specWithPhysics: ScenarioSpec = {
        ...
        gpu_load_profile: inWindowGpuLoadProfile.length > 0 ? inWindowGpuLoadProfile : undefined,
