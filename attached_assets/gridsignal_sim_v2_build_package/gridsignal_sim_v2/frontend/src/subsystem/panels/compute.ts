...
export const computePanel: PanelConfig = {
  deriveData(tick: TickPayload | null, _alert, history: HistoryPoint[]): PanelData {
    ...
    let verdict: string
    if (kube) {
      const mixedFleet = (kube.cluster_metrics?.length ?? 0) > 1
      const unitLabel = mixedFleet ? 'scheduling units' : 'nodes'
      if (kube.power_cap_active) {
        verdict = `Power cap ACTIVE — grid headroom ${kube.headroom_mw.toFixed(1)} MW. `
                + `Scheduler fleet capped at ${fmtNodes(kube.node_count)} ${unitLabel}.`
      } else {
        verdict = `Scheduler demand: ${(kube.utilization * 100).toFixed(0)}% GPU utilisation, `
                + `${fmtNodes(kube.node_count)} ${unitLabel} scheduled. `
    ...
    const kubeRows = kube ? [
      { label: 'Fleet utilisation',    value: `${(kube.utilization * 100).toFixed(1)}%`,       colour: kube.utilization > 0.78 ? AMBER : TEAL },
      { label: 'Active jobs',          value: kube.active_jobs.toString(),                      colour: kube.active_jobs > 0 ? TEAL : undefined },
      { label: 'Admitted units',       value: fmtNodes(kube.admitted_nodes) },
      { label: 'Total units',          value: fmtNodes(kube.node_count) },
      { label: 'GPU capacity',         value: fmtNodes(kube.total_gpu_capacity ?? 0) },
