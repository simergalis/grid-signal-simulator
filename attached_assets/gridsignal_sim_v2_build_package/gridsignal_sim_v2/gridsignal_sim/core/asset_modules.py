...
class GPUModule(AssetModule):
    ...
    def apply_signal(self, signal: WorkloadSignal) -> bool:
        ...
        if signal.event_type == WorkloadEventType.STARTING:
            ...
            # A scheduler snapshot can arrive after the job has already started.
            # Unlike STARTING, RUNNING is an observed live allocation and must
            # contribute its full draw on the next physics tick.
            self._node_counts[signal.job_id] = signal.node_count
            self._job_profiles[signal.job_id] = signal.hardware_profile_id
...
class TurbineModule(AssetModule):
    ...
    def thermal_state(self) -> ThermalState:
        """Current start-readiness tier used by commitment priority and telemetry."""
        return self._thermal_state

    def assign_standby_thermal_state(self, thermal_state: ThermalState) -> None:
        """Assign a promoted readiness label to an off-bus standby unit.

        Promotion is a fleet-policy operation, not a start command.  Updating
        both the live state and first-start configuration ensures the next
        command_start() uses the promoted hot/warm/cold start duration.
        """
        if self.state != TurbineState.OFFLINE or not self.config.hot_standby:
            raise ValueError(
                f"TurbineModule {self.config.asset_id!r}: standby thermal-state "
                "promotion requires an OFFLINE hot_standby unit"
            )
        self.config.initial_thermal_state = thermal_state
...
# ---------------------------------------------------------------------------
# Diesel module -- Addendum H Phase 2
# ---------------------------------------------------------------------------

@dataclass
class DieselModule(AssetModule):
    """One diesel unit with an explicit start outcome and local output curve.

    Fleet policy intentionally does not live here.  The module owns only its
    own start, synchronization, unloading, and restart-lockout state;
    DieselFleetCoordinator owns cross-unit activation and failover.
    """

    config: DieselConfig
    state: DieselState = DieselState.OFFLINE
    _current_output_mw: float = 0.0
    _time_to_online_s: float = 0.0
    _start_success: Optional[bool] = None
    _start_attempted: bool = False
    _run_start_s: float = math.nan
    _last_stop_s: float = math.nan
    _time_since_sync_s: float = 0.0
    _unload_elapsed_s: float = 0.0
    _unload_start_output_mw: float = 0.0
    _cooldown_until_s: float = math.nan
    _last_known_sim_time: float = 0.0
    _rng: random.Random = field(
        default_factory=random.Random,
        repr=False,
        compare=False,
    )
    _resuming_from_unloading: bool = False
    _resume_start_output_mw: float = 0.0
    _resume_elapsed_s: float = 0.0

    def __post_init__(self) -> None:
        if self.config.rated_mw <= 0.0:
            raise ValueError("diesel rated_mw must be greater than zero")
        if not 0.0 <= self.config.p_start <= 1.0:
            raise ValueError("diesel p_start must be between zero and one")
        for name in (
            "delta_t_start_s",
            "residual_ramp_s",
            "min_run_s",
            ...
            if getattr(self.config, name) < 0.0:
                raise ValueError(f"diesel {name} must not be negative")
        if not 0.0 <= self.config.f_block <= 1.0:
            raise ValueError("diesel f_block must be between zero and one")
    ...
    @property
    def start_offset_s(self) -> Optional[float]:
        return self.config.start_offset_s

    @property
    def start_attempted(self) -> bool:
        return self._start_attempted

    @property
    def start_success(self) -> Optional[bool]:
        return self._start_success

    @property
    def time_to_online_s(self) -> Optional[float]:
        if self.state == DieselState.STARTING:
            return self._time_to_online_s
        return 0.0 if self.state == DieselState.SYNCHRONISED else None

    @property
    def is_on_bus(self) -> bool:
        return self.state in (DieselState.SYNCHRONISED, DieselState.UNLOADING)

    def can_start(self, sim_time: float) -> bool:
        """Return whether this unit can accept a new start command."""
        if self.state != DieselState.OFFLINE or self._start_attempted:
            return False
        if not math.isnan(self._cooldown_until_s) and sim_time < self._cooldown_until_s:
            return False
        if (
            not math.isnan(self._last_stop_s)
            and sim_time - self._last_stop_s < self.config.min_down_s
        ):
            return False
        return True
    ...
    def command_start(
        ...
        *,
        ...
        """Accept an OFFLINE start and immediately store its success outcome.

        ``success_override`` is an explicit deterministic test seam.  When it
        is absent, the unit draws once from its configured ``p_start``.
        """
        self._last_known_sim_time = sim_time
        if not self.can_start(sim_time):
            return

        self._start_success = (
            bool(success_override)
            if success_override is not None
            else self._rng.random() < self.config.p_start
        )
        self._start_attempted = True
        self.state = DieselState.STARTING
        self._time_to_online_s = self.config.delta_t_start_s
        self._current_output_mw = 0.0
        self._resuming_from_unloading = False

    def advance(self, sim_time: float, dt_seconds: float) -> None:
        """Advance startup, synchronization output, or controlled unloading."""
        if dt_seconds < 0.0:
            raise ValueError("diesel dt_seconds must not be negative")
        self._last_known_sim_time = sim_time

        if self.state == DieselState.STARTING:
            self._time_to_online_s = max(
                0.0,
                self._time_to_online_s - dt_seconds,
            )
            if self._time_to_online_s <= 0.0:
                self._time_to_online_s = 0.0
                if self._start_success:
                    self.state = DieselState.SYNCHRONISED
                    self._run_start_s = sim_time
                    self._time_since_sync_s = 0.0
                    self._current_output_mw = (
                        self.config.rated_mw * self.config.f_block
                    )
                else:
                    self.state = DieselState.FAILED_START
                    self._current_output_mw = 0.0
            return

        if self.state == DieselState.SYNCHRONISED:
            if self._resuming_from_unloading:
                self._resume_elapsed_s += dt_seconds
                duration = self.config.residual_ramp_s
                fraction = (
                    1.0
                    if duration <= 0.0
                    else min(1.0, self._resume_elapsed_s / duration)
                )
                self._current_output_mw = (
                    self._resume_start_output_mw
                    + (self.config.rated_mw - self._resume_start_output_mw)
                    * fraction
                )
            else:
                self._time_since_sync_s += dt_seconds
                base_mw = self.config.rated_mw * self.config.f_block
                duration = self.config.residual_ramp_s
                fraction = (
                    1.0
                    if duration <= 0.0
                    else min(1.0, self._time_since_sync_s / duration)
                )
                self._current_output_mw = (
                    base_mw
                    + (self.config.rated_mw - base_mw) * fraction
                )
            return

        if self.state == DieselState.UNLOADING:
            self._unload_elapsed_s += dt_seconds
            duration = self.config.cooldown_s
            fraction = (
                1.0
                if duration <= 0.0
                else min(1.0, self._unload_elapsed_s / duration)
            )
            self._current_output_mw = max(
                0.0,
                self._unload_start_output_mw * (1.0 - fraction),
            )
    ...
    def command_stop(self, sim_time: float) -> Optional[str]:
        """Begin controlled unloading, enforcing the diesel minimum run time."""
        self._last_known_sim_time = sim_time
        if self.state != DieselState.SYNCHRONISED:
            return f"diesel_stop_not_allowed_state:{self.state.value}"
        if (
            not math.isnan(self._run_start_s)
            and sim_time - self._run_start_s < self.config.min_run_s
        ):
            remaining = self.config.min_run_s - (sim_time - self._run_start_s)
            return f"diesel_min_run_not_elapsed:{remaining:.3f}s"
        self.state = DieselState.UNLOADING
        self._unload_elapsed_s = 0.0
        self._unload_start_output_mw = self._current_output_mw
        return None

    def resume_from_unloading(self) -> None:
        """Reverse an in-flight unload without resetting the run start time."""
        if self.state != DieselState.UNLOADING:
            return
        self.state = DieselState.SYNCHRONISED
        self._resuming_from_unloading = True
        self._resume_start_output_mw = self._current_output_mw
        self._resume_elapsed_s = 0.0

    def complete_unloading(self, sim_time: float) -> None:
        """Finish unloading and enter the diesel restart lockout."""
        if self.state != DieselState.UNLOADING:
            return
        self._last_known_sim_time = sim_time
        self.state = DieselState.OFFLINE
        self._current_output_mw = 0.0
        self._last_stop_s = sim_time
        self._cooldown_until_s = sim_time + self.config.min_down_s

    def reset_for_new_episode(self) -> None:
        """Clear one-shot start state after a fully completed episode."""
        if self.state != DieselState.OFFLINE:
            raise ValueError("diesel unit must be OFFLINE before episode reset")
        self._start_attempted = False
        self._start_success = None
        self._time_to_online_s = 0.0
        self._run_start_s = math.nan
        self._time_since_sync_s = 0.0
        self._unload_elapsed_s = 0.0
        self._unload_start_output_mw = 0.0
        self._resuming_from_unloading = False
...
class BessModule(AssetModule):
    ...
    def bridging_available_mw(self, island_mode: IslandMode) -> float:
        """Return this unit's instantaneous anchor-adjusted power ceiling.

        The return value is the rated power minus ``P_anchor_reserve`` when
        both conditions hold:
          1. This unit is the designated grid-forming anchor
             (``grid_forming=True``).
          2. The site is in ISLANDED mode
             (``island_mode=IslandMode.ISLANDED``).

        In any other combination (grid-following unit, or grid-tie mode),
        ``P_anchor_reserve`` is zero and the full rated power is available for
        bridging.  This method intentionally reports a power ceiling only; it
        does not apply SoC/energy bounding.  SoC and energy limits are applied
        elsewhere by ``cover_shortfall()``, the energy-ceiling calculation in
        ``evaluate_tick()``, and ``max_sustainable_seconds()``.

        Conservative default: ``grid_forming=False`` means most units have no
    ...
    def cover_shortfall(
        ...
        soc_floor_mwh — unconditional minimum SoC retained for Addendum I
