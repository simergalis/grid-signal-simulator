...
from .power_balance import BalanceResult as PhysicalBalanceResult
...
class SiteConfig:
    ...
    # Optional PCC real-power import ceiling.  None preserves the historical
    ...
    # cascade_commit_fraction: when set, the commitment engine triggers the next
    # standby-turbine start when the LAST on-bus active (non-hot-standby) turbine's
    # output reaches this fraction of its own rated MW.
    # None = use fleet-utilisation threshold (commit_utilisation) instead.
    ...
    # Deprecated compatibility field.  The former fuel-cell-output crossing
    # trigger was removed; this value is retained so older scenario payloads
    # continue to validate, but it has no runtime effect.
    fuel_cell_turbine_commit_fraction: Optional[float] = None
...
class DieselState(str, Enum):
    """Diesel unit states, including an explicit failed-start outcome."""
...
@dataclass
class DieselConfig:
    """Runtime configuration for one materialized diesel unit."""
...
class KubeMetrics:
    ...
    cap_gate_deferred_count: int = 0  # admissions deferred by contracted-capacity gate this tick
    queued_jobs: int = 0     # jobs currently sitting in the reorder buffer (observed, not yet admitted)
...
class TickResult:
    ...
    insufficient_reserve_proposal_id: Optional[str] = None
    contingency_release_alert: bool = False
    contingency_release_turbine_id: Optional[str] = None
    contingency_release_coverage_state: Optional[str] = None
    contingency_release_shed_mw: float = 0.0
    contingency_release_sim_time_s: Optional[float] = None
    ...
    # Live fuel-cell readiness telemetry.  The countdown is authoritative when
    # the aggregate array is WARMING; it is 0.0 in HOT_STANDBY and null when
    ...
    diesel_enabled: bool = False
    diesel_output_mw: float = 0.0
    diesel_n_synced: int = 0
    diesel_n_active_target: int = 0
    diesel_n_standby_available: int = 0
    diesel_n_out_of_service: int = 0
    diesel_fleet_state: str = "idle"
    diesel_activation_episode_id: Optional[str] = None
    diesel_time_in_state_s: float = 0.0
    diesel_fuel_remaining_gal: float = 0.0
    diesel_fuel_runtime_hours_remaining: Optional[float] = None
    diesel_insufficient_start_shortfall_mw: Optional[float] = None
    ...
    # Optional time-indexed forecast for scripted future workload changes.
    # Each entry contains minutes_from_now and the full-TDP compute forecast
    # expected at that planning sample. Empty for direct/headless runs.
    planning_forecast_mw: tuple[dict[str, float], ...] = field(default_factory=tuple)
    ...
    # Authoritative physical supply/load ledger check.  Attached once by
    # evaluate_tick() and reused by transport, persistence, monitoring, and
    # the completed-run gate; consumers must not reconstruct it.
    physical_balance: Optional[PhysicalBalanceResult] = None
    ...
    # capacity_gate_deferred_count: number of Kube jobs deferred this tick by the
    #   contracted-capacity gate (compute MW ceiling).  0 when gate is inactive or
    #   no kube_config is present.  Positive → operator should see amber alert.
    # capacity_gate_headroom_mw: design_peak_load_mw − p_compute_demand_mw.
    #   Positive → headroom remaining; negative → compute demand already over cap.
    #   0.0 when design_peak_load_mw is unset (0.0) — frontend should suppress display.
    capacity_gate_deferred_count: int   = 0
    capacity_gate_headroom_mw:    float = 0.0
    ...
    # Runtime-sourced commitment trigger from CommitmentConfig.from_catalogue().
    commit_utilisation_threshold: Optional[float] = None
