    ...
    if _cascade_frac is not None:
        ...
        if _active_on_bus:
            ...
            if _lead_turbine.output_mw() >= _threshold_mw:
                ...
                _hs_units = [
                    t for t in state.turbines
                    if t.config.hot_standby and t.state == TurbineState.OFFLINE
                ]
                if _hs_units and state._pending_start.is_empty:
                    _next_hs = _hs_units[0]  # first standby unit in roster order
                    ...
                    if _next_hs.state == TurbineState.STARTING:
                        ...
                        _log.info(
                            ...
                            "sim_time=%.1f (lead %r %.2f MW ≥ %.2f MW threshold)",
