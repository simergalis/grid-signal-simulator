...
from .dispatch import (
    ...
    ContingencyReleaseAlert, CurtailmentProposal, CurtailmentTier, DispatchArbitrator,
...
from .diesel_fleet import DieselFleetCoordinator
from .scada_layer import CommandType, SimulatedPMS, SimulatedScadaLayer
from .models import (
    ContingencyCoverage, ContingencyState, DataQualityTag,
    GENERIC_FALLBACK_PROFILE, IslandMode, KubeClusterMetrics, KubeMetrics,
    SiteConfig, ThermalState, TickResult, WorkloadEventType, WorkloadSignal,
)
...
from .power_balance import (
    ...
    evaluate_physical_balance as _evaluate_physical_balance,
...
class SimulationState:
    ...
    diesel_units: list[DieselModule] = field(default_factory=list)
    # Diesel fleet-level settings are copied from the scenario block at
    # construction time so the coordinator remains owned by SimulationState.
    diesel_debounce_s: float = 1.0
    diesel_restore_hold_s: float = 300.0
    diesel_fuel_burn_gal_per_hr_per_unit_at_full_load: float = 230.0
    diesel_min_fuel_runtime_hours: float = 48.0
    ...
    diesel_fleet_coordinator: DieselFleetCoordinator = field(init=False)
    tick_index: int = 0
    # Diesel output is coordinated after BESS arbitration.  The previous
    # tick's output is therefore the only diesel contribution available when
    # the next tick's emergency BESS target is calculated.
    _prev_diesel_output_mw: float = field(default=0.0, init=False)
    ...
    _pending_contingency_release_alert: ContingencyReleaseAlert | None = None
    ...
    # Contingency-triggered standby release is deliberately separate from
    # evaluate_commitment().  The condition is armed again only after one
    # complete COVERED tick, so one sustained contingency episode releases at
    # most one hot standby even when the first released unit is still starting.
    _contingency_release_cond: SustainedCondition = field(init=False)
    _contingency_release_armed: bool = field(default=True, init=False)
    ...
    def __post_init__(self) -> None:
        ...
        self.arbitrator = DispatchArbitrator(
            self.turbines,
            self.bess_units,
            self.site,
            diesel_units=self.diesel_units,
        )
        self.diesel_fleet_coordinator = DieselFleetCoordinator(
            diesel_units=self.diesel_units,
            debounce_s=self.diesel_debounce_s,
            restore_hold_s=self.diesel_restore_hold_s,
            fuel_burn_gal_per_hr_per_unit_at_full_load=(
                self.diesel_fuel_burn_gal_per_hr_per_unit_at_full_load
            ),
            min_fuel_runtime_hours=self.diesel_min_fuel_runtime_hours,
        )
        ...
        self._contingency_release_cond = SustainedCondition(
            threshold_s=self._commit_cfg.contingency_release_confirm_s
        )
    ...
    def apply_workload_signal(self, signal: WorkloadSignal, dt_lead_seconds: float) -> None:
        ...
        elif signal.event_type == WorkloadEventType.RUNNING:
            # RUNNING is an observed allocation from an external scheduler,
            # not an advance notice.  GPUModule.apply_signal() has already
            # installed it at full draw; register its cooling envelope without
            # staging a future turbine response.
            self.cooling.register_job_start(signal.job_id, signal.timestamp)
...
def _release_hot_standby_for_contingency(
    state: SimulationState,
    coverage: ContingencyCoverage,
    *,
    sim_time: float,
    dt_seconds: float,
) -> ContingencyReleaseAlert | None:
    """Release at most one hot standby after sustained real N-1 risk.

    This is intentionally not part of evaluate_commitment().  The ordinary
    commitment path must continue to exclude hot-standby units from its
    offline candidates; this path is the inverse, contingency-only action.
    """
    risk_state = coverage.state in (
        ContingencyState.COVERED_WITH_SHED,
        ContingencyState.CANNOT_CARRY,
    )
    state._contingency_release_cond.update(risk_state, dt_seconds)

    # One complete COVERED tick re-arms the episode.  A bad condition never
    # re-arms itself, even after the first released unit reaches STARTING or
    # SYNCHRONISED.
    if coverage.state == ContingencyState.COVERED:
        state._contingency_release_armed = True

    if (
        not risk_state
        or not state._contingency_release_cond.met
        or not state._contingency_release_armed
        or not state._pending_start.is_empty
    ):
        return None

    hot_standby_units = [
        turbine
        for turbine in _offline_turbines_by_thermal_priority(state.turbines)
        if turbine.config.hot_standby
    ]
    if not hot_standby_units:
        return None

    turbine = hot_standby_units[0]
    turbine.config.hot_standby = False
    turbine.command_start(sim_time)
    if turbine.state != TurbineState.STARTING:
        # Preserve the reserve label when the turbine's own start guards reject
        # the command; no release was actually made and a later tick may retry.
        turbine.config.hot_standby = True
        return None

    state._pending_start.record_start(turbine.config.asset_id, sim_time)
    state._contingency_release_armed = False
    return ContingencyReleaseAlert(
        turbine_id=turbine.config.asset_id,
        coverage_state=coverage.state.value,
        shed_mw=float(coverage.shed_required_mw),
...
def evaluate_tick(state: SimulationState, clock: SimClock) -> TickResult:
    ...
    fuel_cell_state = (
        state.fuel_cell_module.state.value
        if state.fuel_cell_module is not None
        else None
    )
    fuel_cell_time_to_ready_s = (
        state.fuel_cell_module.time_to_ready_s
        if state.fuel_cell_module is not None
        else None
    )
    ...
    # it is correct for loading but can understate the N-1 reserve requirement.
    _commitment_demand_mw = max(
        0.0,
    ...
    _bess_reconciled_reserve_mwh: list[float] = []
    for _bess in state.bess_units:
        ...
        # Reconcile the configured normal-dispatch reserve with the diesel
        # synchronization floor in one place (DispatchArbitrator).  The
        # reconciled MWh threshold drives both the upstream normal/reserve split
        # and the downstream physical discharge guard.
        _reconciled_reserved_energy_mwh = state.arbitrator.soc_floor_mwh(_bess)
        _bess_reconciled_reserve_mwh.append(_reconciled_reserved_energy_mwh)
        if _reconciled_reserved_energy_mwh <= 0.0:
    ...
    if _normal_bess_energy_exhausted:
        ...
        # available to this model: online turbine output, measured fuel-cell
        ...
        _emergency_release_authorized = _emergency_gap_mw > 0.0
    ...
    # authorizes that retained energy for this tick only; no other contingency
    # state is introduced here.
    _bess_soc_floors_mwh = (
        [0.0] * len(_bess_reconciled_reserve_mwh)
        if _emergency_release_authorized
    ...
    _turbine_surplus_before_bess_mw = max(
        0.0,
        turbine_output_mw + fuel_cell_output_mw - _p_dispatch_droop_mw,
    )
    ...
    # The fuel cell has already been advanced and measured above.  It is not
    # allowed to jump to a residual setpoint in this orchestration layer.
    _fuel_cell_setpoint_mw = fuel_cell_output_mw
    ...
    if (
        _commit_decision.action == "commit"
        ...
        _commit_decision.action == "decommit"
        and _commit_decision.target_unit_id is not None
    ):
    ...
    # Remaining gap passed to the curtailment ladder accounts for all measured
    # dispatch sources, including diesel coordinated above.
    _remaining_gap_mw = max(0.0, _gap_before_diesel_mw - diesel_output_mw)
    ...
    _release_alert = _release_hot_standby_for_contingency(
        state,
        _contingency_coverage,
        sim_time=sim_time,
        dt_seconds=dt_seconds,
    )
    if _release_alert is not None:
        state._pending_contingency_release_alert = _release_alert
    ...
    contingency_release_alert = state._pending_contingency_release_alert
    state._pending_contingency_release_alert = None
    ...
    # P_gen(t) = turbines + BESS + fuel cell + diesel (actual measured)
    #             + renewables.
    ...
    # The fixed-target FC contributes its measured output; the thermal fleet and
    # BESS receive only the remaining dispatch requirement.
    _p_commanded_mw = (
        _thermal_dispatch_target_mw
        + _bess_setpoint_mw
    ...
    _d4_balance_defect_mw = _balance_defect_mw(_balance_terms)

    # Authoritative physical per-tick ledger invariant. All terms below were
    # already produced by the simulation path; this is the one place they are
    # assembled and evaluated. A generation deficit remains a real residual and
    # p_imbalance_mw; it is never hidden inside the explicit-shed field.
    _p_losses_mw = 0.0  # no loss model is currently represented by the simulator
    _physical_balance = _evaluate_physical_balance(
        _BalanceTerms(
            p_generation_mw=_p_gen_mw,
            p_demand_mw=p_demand_mw,
            p_unserved_mw=_p_unserved_mw,
            p_served_mw=_p_served_mw,
            grid_exchange_mw=-_grid_exchange_mw,
            p_losses_mw=_p_losses_mw,
            island_mode=_BAL_ISLANDED if _islanded else _BAL_GRID_TIE,
            p_turbine_mw=turbine_output_mw,
            p_bess_mw=bess_output_mw,
            p_fuel_cell_mw=fuel_cell_output_mw,
            p_renewable_mw=p_renewable_mw,
        ),
        tolerance_mw=_sp.value("balance_defect_tolerance_mw"),
    )

    if not _physical_balance.passed:
        _log.warning(
            "physical balance invariant does not close: %+.4g MW "
            "(generation=%.4f, served=%.4f, unserved=%.4f, "
            "grid_import=%.4f, losses=%.4f, islanded=%s)",
            _physical_balance.defect_mw,
            _physical_balance.term_breakdown["generation_mw"],
            _physical_balance.term_breakdown["served_load_mw"],
            _physical_balance.term_breakdown["unserved_load_mw"],
            _physical_balance.term_breakdown["grid_import_mw"],
            _physical_balance.term_breakdown["losses_mw"],
            _islanded,
        )
    ...
    return TickResult(
        ...
        diesel_n_active_target=_diesel_snapshot.n_active,
        diesel_n_standby_available=_diesel_snapshot.n_standby_available,
        diesel_n_out_of_service=_diesel_snapshot.n_out_of_service,
        diesel_fleet_state=_diesel_snapshot.state,
        diesel_activation_episode_id=_diesel_snapshot.episode_id,
        diesel_time_in_state_s=_diesel_snapshot.time_in_state_s,
        diesel_fuel_remaining_gal=_diesel_snapshot.fuel_remaining_gal,
        diesel_fuel_runtime_hours_remaining=(
            _diesel_snapshot.fuel_runtime_hours_remaining
        ),
        diesel_insufficient_start_shortfall_mw=(
            _diesel_snapshot.shortfall_mw
            if _diesel_snapshot.state == "insufficient_start"
            else None
        ),
        bess_soc_fraction=(state.bess_units[0].soc_fraction if state.bess_units else 1.0),
        confidence=confidence,
        insufficient_reserve_alert=alert_fired,
        contingency_release_alert=contingency_release_alert is not None,
        contingency_release_turbine_id=(
            contingency_release_alert.turbine_id
            if contingency_release_alert is not None else None
        ),
        contingency_release_coverage_state=(
            contingency_release_alert.coverage_state
            if contingency_release_alert is not None else None
        ),
        contingency_release_shed_mw=(
            contingency_release_alert.shed_mw
            if contingency_release_alert is not None else 0.0
        ),
        contingency_release_sim_time_s=(
            contingency_release_alert.fires_at_sim_time
            if contingency_release_alert is not None else None
        ),
        ...
        physical_balance=_physical_balance,
        ...
        commit_utilisation_threshold=getattr(
            getattr(state, "_commit_cfg", None), "commit_utilisation", None
        ),
