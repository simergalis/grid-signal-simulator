...
class KubeConfig:
    ...
    # Job size distribution (Gaussian, clipped to [min_job_nodes, max_job_nodes]).
    # None preserves the legacy max_nodes/2 fallback for existing scenarios.
    ...
    max_job_nodes: Optional[int] = None  # per-job cap; None = legacy max_nodes/2
    ...
    gpus_per_unit: int = 1
    # Non-empty only on the new multi-cluster path.  Legacy event IDs remain
    # byte-for-byte stable for existing deterministic replay fixtures.
    event_id_prefix: str = ""
    ...
    # ── Contracted capacity gate ───────────────────────────────────────────────
    # When set, the admission loop rejects any new job whose estimated compute
    # draw (node_count × rated_kw_per_node / 1000) would push the fleet-wide
    # committed compute MW above this ceiling.  Deferred jobs are re-queued
    # with a "-capdefer-N" suffix — never dropped — and retried every tick.
    #
    # Ceiling is interpreted as COMPUTE-only MW (IT load without PUE cooling
    # overhead), matching design_peak_load_mw semantics in TickResult.
    # Set to None to disable (default; no existing tests affected).
    capacity_ceiling_mw: Optional[float] = None
...
class KubeDemandAgent:
    ...
    def tick(
        ...
        while self._next_arrival_sim_time <= sim_time:
            self._job_counter += 1
            _prefix = f"{self.config.event_id_prefix}-" if self.config.event_id_prefix else ""
            event_id = f"{_prefix}kube-job-{self._job_counter}"

            # Gang size: Gaussian, clipped to the cluster-specific job cap.
            # A policy cap may exceed a small fleet's total capacity (e.g. the
            # SJ-1 Ray policy is 42 racks while that cluster has 21 racks);
            # the fleet capacity check below remains the final hard bound.
            _job_cap_units = (
                min(self.config.max_job_nodes, self.config.max_nodes)
                if self.config.max_job_nodes is not None
                else self.config.max_nodes // 2
            )
            node_count = int(round(max(
                float(self.config.min_job_nodes),
                min(
                    float(_job_cap_units),
                    self._rng.gauss(
        ...
        for pa in ready:
            ...
            # ── Contracted capacity gate ──────────────────────────────────────
            # When capacity_ceiling_mw is set, block any admission that would
            # push committed compute MW above the contracted site cap.
            # Committed compute MW = (active jobs + newly admitted this tick +
            # cross-agent already admitted) × rated_kw_per_node / 1000.
            # Deferred jobs are re-queued with a "-capdefer-N" suffix so they
            # survive until headroom opens; they are never dropped.
            if self.config.capacity_ceiling_mw is not None:
                _kw_per_node = self.config.rated_kw_per_node
                _committed_compute_mw = (
                    sum(j.node_count for j in self._active_jobs) * _kw_per_node / 1000.0
                    + sum(j.node_count for j in newly_admitted) * _kw_per_node / 1000.0
                    + already_admitted_nodes * _kw_per_node / 1000.0
                )
                _new_job_compute_mw = pa.node_count * _kw_per_node / 1000.0
                if _committed_compute_mw + _new_job_compute_mw > self.config.capacity_ceiling_mw:
                    _cap_base_id = pa.event_id.split("-capdefer")[0]
                    _cap_count = pa.requeue_count + 1
                    _cap_retry_id = f"{_cap_base_id}-capdefer-{_cap_count}"
                    if _cap_retry_id not in self._seen_event_ids:
                        self._reorder_buffer.append(_PendingAdmission(
                            event_id=_cap_retry_id,
                            node_count=pa.node_count,
                            hardware_profile_id=pa.hardware_profile_id,
                            observed_at=sim_time + dt_seconds,
                            event_timestamp=sim_time + dt_seconds,
                            duration_s=pa.duration_s,
                            tenant_id=pa.tenant_id,
                            scheduler_type=pa.scheduler_type,
                            first_queued_at=pa.first_queued_at,
                            requeue_count=pa.requeue_count,  # don't count cap-defers as power-cap requeues
                        ))
                    cap_gate_deferred += 1
                    _log.debug(
                        "kube: CAP-GATE defer %s (%d nodes, committed=%.2f MW, "
                        "new=%.2f MW, ceiling=%.2f MW) → capdefer-%d",
                        _cap_base_id, pa.node_count,
                        _committed_compute_mw, _new_job_compute_mw,
                        self.config.capacity_ceiling_mw, _cap_count,
                    )
                    continue
        ...
        if not self._started or total_nodes != self._last_total_nodes:
            ...
            # A KubeDemandAgent reports the current aggregate allocation for one
            # tenant's cluster partition.  STARTING and every later SCALE must
            # keep the same job_id so GPUModule can replace that aggregate count
            # instead of treating each update as another concurrent cluster.
            #
            # tenant_id makes identities distinct when the factory creates the
            # A/B/C agents for one shared physical fleet.  event_id remains
            # tick-specific for traceability and event deduplication.
            _signal_prefix = (
                f"{self.config.cluster_id}-" if self.config.cluster_id else ""
            )
            signals.append(WorkloadSignal(
                event_id=f"{_signal_prefix}kube-signal-t{int(sim_time * 10)}",
                job_id=f"kube-admission-{self.config.cluster_id or self.config.tenant_id}",
                ...
                tenant_id=self.config.tenant_id,
                cluster_id=self.config.cluster_id or None,
                scheduler_type=self.config.scheduler_type,
                capacity_unit=self.config.capacity_unit,
                gpus_per_unit=self.config.gpus_per_unit,
            ))
            self._started = True
        ...
        metrics = KubeMetrics(
            ...
            cap_gate_deferred_count=cap_gate_deferred,
            ...
            cluster_metrics=(
                KubeClusterMetrics(
                    cluster_id=self.config.cluster_id or self.config.tenant_id,
                    tenant_id=self.config.tenant_id,
                    scheduler_type=self.config.scheduler_type,
                    hardware_profile_id=self.config.hardware_profile_id,
                    capacity_unit=self.config.capacity_unit,
                    gpus_per_unit=self.config.gpus_per_unit,
                    max_units=self.config.max_nodes,
                    scheduled_units=total_nodes,
                    admitted_units=admitted_nodes,
                    gpu_capacity=self.config.max_nodes * self.config.gpus_per_unit,
                    rated_capacity_mw=round(
                        self.config.max_nodes * self.config.rated_kw_per_node / 1000.0,
                        4,
                    ),
                    utilization=utilization,
                ),
            ),
            total_gpu_capacity=self.config.max_nodes * self.config.gpus_per_unit,
