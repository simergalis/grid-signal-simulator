...
class CommitmentConfig:
    ...
    contingency_release_confirm_s (12) Confirmation window for contingency-triggered
                                      hot-standby release.  Engineering estimate;
                                      tune against operational data.
    """
    ...
    contingency_release_confirm_s: float  # engineering estimate; not yet validated
    ...
    def from_catalogue(cls) -> "CommitmentConfig":
        ...
        return cls(
            ...
            contingency_release_confirm_s=float(
                _sp_value("contingency_release_confirm_s")
            ),
        )
...
def evaluate_commitment(
    ...
    dt_s: float,
    sim_time: float,
    force_commit_trigger: bool = False,
    decommit_enabled: bool = True,
    decommit_reason_context: Optional[str] = None,
) -> CommitmentDecision:
    ...
    # ── Step 3: Commit path ────────────────────────────────────────────────
    # force_commit_trigger: set by cascade policy (site.cascade_commit_fraction) when
    # the last on-bus active turbine's output crosses the per-unit threshold.
    commit_trigger = force_commit_trigger or floor_violated or utilisation >= cfg.commit_utilisation
    ...
    decommit_blocked_by: Optional[str] = None
    if on_bus and not commit_cond.met and decommit_enabled:
        ...
        if floor_violated:
            decommit_blocked_by = "reserve floor is currently violated"
        elif not floor_ok_without:
            decommit_blocked_by = (
                "reserve floor would be violated after removing the selected turbine"
            )
        elif utilisation > cfg.decommit_utilisation:
            decommit_blocked_by = (
                f"utilisation U={utilisation:.3f} exceeds "
                f"decommit threshold {cfg.decommit_utilisation}"
            )
        elif u_without > cfg.decommit_post_removal_max:
            decommit_blocked_by = (
                f"post-removal utilisation U_without={u_without:.3f} exceeds "
                f"{cfg.decommit_post_removal_max}"
            )
    ...
    if decommit_cond.met and decommit_candidate is not None:
        context = (
            f"; {decommit_reason_context}"
            if decommit_reason_context
            else ""
        )
        return CommitmentDecision(
            action="decommit",
    ...
    return CommitmentDecision(
        ...
        blocked_by=decommit_blocked_by,
        floor_mw=floor_mw, floor_violated=floor_violated,
    )
