...
def _minimal_state(
    *,
    ...
    site = SiteConfig(frequency_nominal_hz=50.0, power_factor=0.85, site_id="test-site", pue_base=1.03, uncalibrated=False,
...
def test_dt_lead_next_s_is_min_not_sum():
    ...
    """
    site = SiteConfig(frequency_nominal_hz=50.0, power_factor=0.85, site_id="s", pue_base=1.0, uncalibrated=False)
...
def test_turbine_ramp_credit_present_in_payload_with_active_ramp():
    ...
    site = SiteConfig(frequency_nominal_hz=50.0, power_factor=0.85, site_id="test-ramp-credit", pue_base=1.0, uncalibrated=False,
                      island_mode=IslandMode.ISLANDED)
    hw = {"profile_a": HardwareProfile("profile_a", rated_kw=10.0)}
    gpu = GPUModule(asset_id="gpu-0", site=site, hardware_library=hw)
    turbine = TurbineModule(
