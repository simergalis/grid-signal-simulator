...
class TestRankingSort:
    ...
    def test_cheapest_is_rank_1(self) -> None:
        sources = [
            ...
            _make_source("turbine-1", PowerSourceType.TURBINE,
                         marginal_cost_mwh=55.0),
            _make_source("fc-1", PowerSourceType.FUEL_CELL,
        ...
        assert result.ranked_sources[1].source_id == "turbine-1"
        assert result.ranked_sources[1].rank == 2
        assert result.ranked_sources[2].source_id == "fc-1"
        assert result.ranked_sources[2].rank == 3
        assert result.ranked_sources[3].source_id == "grid-1"
        assert result.ranked_sources[3].rank == 4
