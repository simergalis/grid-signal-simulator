...
@pytest.mark.asyncio
async def test_get_tick_dicts_rehydrates_bess_escalation_evidence(tmp_path: Path) -> None:
    """Persisted playback retains the same escalation evidence as live ticks."""
    sink = SqlitePersistedTimeseriesSink(tmp_path / "bess-escalation.db")
    await sink.start()
    tick = replace(
        _make_tick("run-bess-escalation", 1),
        bess_escalation_active=True,
        bess_escalation_reason="turbine_catchup",
        bess_bridging_available_mw=2.5,
        bess_bridging_floor_mw=4.5,
        bess_material_discharge_threshold_mw=1.5,
        bess_discharge_sustained_s=30.0,
        turbine_observed_ramp_mw_per_s=0.1,
        turbine_estimated_time_to_close_s=50.0,
    )
    await sink.append(tick)
    rows = await sink.get_tick_dicts("run-bess-escalation")

    assert len(rows) == 1
    assert rows[0]["bess_escalation_active"] is True
    assert rows[0]["bess_escalation_reason"] == "turbine_catchup"
    assert rows[0]["bess_bridging_available_mw"] == pytest.approx(2.5)
    assert rows[0]["bess_discharge_sustained_s"] == pytest.approx(30.0)
    assert rows[0]["turbine_estimated_time_to_close_s"] == pytest.approx(50.0)
    await sink.stop()
