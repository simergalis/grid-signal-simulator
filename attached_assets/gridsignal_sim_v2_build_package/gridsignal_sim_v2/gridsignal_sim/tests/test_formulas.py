...
from core.models import BessConfig, HardwareProfile, SiteConfig, SolarConfig, TurbineConfig, TurbineState, WorkloadEventType, WorkloadSignal, WorkloadClass
...
def test_tc04_fuel_cell_only_reserve_credit():
    """Fuel-cell ramp credit reduces the TC-10-style peak shortfall."""
    fuel_cell = FuelCellModule(
        FuelCellConfig(
            asset_id="fc-tc04",
            rated_mw=25.0,
            ramp_rate_mw_per_s=0.2,
        ),
    ...
    arbitrator = DispatchArbitrator(
        ...
        site=SiteConfig(
            ...
            power_factor=0.85,
            site_id="s-tc04-fc-only",
        ),
        fuel_cell_units=[fuel_cell],
    )

    alert, credit_mw, shortfall_mw = arbitrator.stage_for_predicted_step(
        delta_p_mw=20.0,
    ...
    assert math.isclose(alert.gap_duration_s, 70.0, abs_tol=1e-6)
    assert math.isclose(alert.shortfall_mw, 14.0, abs_tol=1e-6)
    assert math.isclose(credit_mw, 6.0, abs_tol=1e-6)
    assert math.isclose(shortfall_mw, 14.0, abs_tol=1e-6)


def test_tc05_mixed_turbine_and_fuel_cell_reserve_credit():
    """TC-05: mixed fleets sum turbine and RUNNING fuel-cell ramp credit."""
    turbine = TurbineModule(
        TurbineConfig(
            asset_id="t-tc04-mixed",
            rated_mw=25.0,
    ...
    fuel_cell = FuelCellModule(
        FuelCellConfig(
            asset_id="fc-tc04-mixed",
            rated_mw=25.0,
            ramp_rate_mw_per_s=0.1,
        ),
        state=FuelCellState.RUNNING,
    )
    arbitrator = DispatchArbitrator(
    ...
    assert alert is not None
    assert math.isclose(alert.gap_duration_s, 20.0 / 0.3 - 30.0, abs_tol=1e-6)
    assert math.isclose(alert.shortfall_mw, 11.0, abs_tol=1e-6)
    assert math.isclose(credit_mw, 9.0, abs_tol=1e-6)
    assert math.isclose(shortfall_mw, 11.0, abs_tol=1e-6)
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "MISSING_POLICY: Cost-ranked economic dispatch ($38/MWh BESS vs $55/MWh "
        "turbine) has no reserve-preservation mechanism. Extended trace (600s) "
        "shows turbine output converges to a stable asymptote at "
        "(P_dispatch_required - BESS_rated_mw) rather than covering full demand; "
        "BESS holds at its rated ceiling indefinitely instead of tapering once "
        "the turbine has physical headroom. D10's docstring describes the "
        "intended bridge-then-taper architecture correctly -- the dispatch "
        "arbitrator does not yet implement it. See DR-2026-09-02-BESS-POLICY. "
        "Do not remove this xfail by loosening the test's assertions; remove it "
        "only once a reserve-preservation dispatch policy is implemented and "
        "specified."
    ),
)
