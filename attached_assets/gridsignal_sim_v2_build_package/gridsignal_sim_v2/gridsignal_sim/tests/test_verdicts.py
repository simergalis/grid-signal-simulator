...
class TestEvaluateVerdictGapHandling:
    ...
    def test_pue_base_declared_range_uses_catalogue_bounds(self) -> None:
        entry = _sp.get("pue_base")
        assert entry.min is not None and entry.max is not None
        bounds = (float(entry.min), float(entry.max))
        assertion = PueBaseInDeclaredRangeAssertion()

        passing = evaluate_verdict(
            assertions=[assertion],
            rows=[],
            dropped_ticks=0,
            pue_base=1.074,
            pue_base_bounds=bounds,
        )
        assert passing.overall == "PASS"
        assert passing.assertions[0].detail == (
            "pue_base 1.074 within PARAM-06 range [1.01, 1.1]"
        )

        failing = evaluate_verdict(
            assertions=[assertion],
            rows=[],
            dropped_ticks=0,
            pue_base=1.2,
        ...
        assert failing.assertions[0].detail == (
            "pue_base 1.2 outside PARAM-06 range [1.01, 1.1]"
        )
...
class TestFullRunVerdicts:
    ...
    @pytest.mark.parametrize(
        ("runtime_pue_base", "expected_status"),
        [(1.074, "PASS"), (1.2, "FAIL")],
    )
    def test_run_manager_uses_runtime_pue_and_catalogue_bounds(
        self,
        runtime_pue_base: float,
        expected_status: str,
    ) -> None:
        run_id = f"run-pue-guard-{expected_status.lower()}"
        ...
        ctx.sim_state.site.pue_base = runtime_pue_base
        ctx.assertions = [PueBaseInDeclaredRangeAssertion()]
        ctx.scenario_name = run_id
        ctx.sink = InMemoryTimeseriesSink()
        ...
        assert completed is not None
        assert completed.verdict.overall == expected_status
        assert completed.verdict.assertions[0].status == expected_status
