...
def _bess_reserve_tick(
    ...
    *,
    ...
    return ctx, _run_tick(ctx.sim_state, sim_time=0.0, dt=5.0)


def test_fixed_fuel_cell_baseload_is_absorbed_before_normal_bess_dispatch() -> None:
    ctx, tick = _bess_reserve_tick(
        _bess_reserve_spec(), bess_soc_fraction=0.95
    )
    ...
    expected_fc_mw = min(
        ...
        + module.config.ramp_rate_mw_per_s * 5.0,
        module.target_output_mw,
    )
    assert tick.fuel_cell_output_mw == pytest.approx(expected_fc_mw)
    assert tick.bess_output_mw < -0.1
    assert tick.p_unserved_mw == pytest.approx(0.0, abs=1e-6)


def test_fixed_fuel_cell_output_does_not_release_bess_reserve() -> None:
    ctx, tick = _bess_reserve_tick(
        _bess_reserve_spec(), bess_soc_fraction=0.92
    )
    ...
    assert tick.fuel_cell_output_mw == pytest.approx(expected_fc_mw)
    assert tick.bess_output_mw < -0.1
    assert tick.bess_soc_fraction > 0.92
    assert tick.p_unserved_mw == pytest.approx(0.0, abs=1e-6)
...
def test_sj1_validation_burst_uses_fixed_fuel_cell_and_bess_dispatch() -> None:
    """The scheduled surge uses fixed FC output and the BESS residual."""
    ...
    assert all(tick.bess_output_mw < -0.01 or tick.bess_output_mw > 0.1
               for tick in validation_ticks)
    assert max(tick.fuel_cell_output_mw for tick in validation_ticks) == pytest.approx(24.0)
    assert min(tick.bess_output_mw for tick in validation_ticks) < -0.01
    assert max(tick.bess_output_mw for tick in validation_ticks) > 0.1
    assert all(tick.grid_exchange_mw >= -1e-6 for tick in validation_ticks)
    assert all(tick.p_unserved_mw == pytest.approx(0.0, abs=1e-6)
               for tick in validation_ticks)


def test_sj1_three_overpeak_bursts_respect_supply_limits_and_recover() -> None:
    """Each authored peak respects the PCC cap and recovers before the next."""
    spec_data = _load_raw(_SCENARIO_PATH)
    ctx = build_run_context_from_spec("sj1-three-deficit-peaks", spec_data)
    ctx.playback_speed = 0.0
    sink = InMemoryTimeseriesSink()
    ctx.sink = sink
    manager = RunManager(WebSocketHub())
    ...
    for start_s, end_s in ((300.0, 360.0), (1050.0, 1110.0), (1800.0, 1860.0)):
        peak_ticks = [
            tick for tick in sink.rows
            if start_s <= tick.sim_time_seconds < end_s
        ]
        assert peak_ticks
        assert all(
            tick.p_unserved_mw == pytest.approx(0.0, abs=1e-6)
            for tick in peak_ticks
        )
        assert min(tick.grid_exchange_mw for tick in peak_ticks) >= -5.0 - 1e-6

        recovery_ticks = [
            tick for tick in sink.rows
            # Ticks are timestamped at interval end, so the tick stamped at
            # end_s still reflects the preceding five-second peak interval.
            if end_s + 5.0 <= tick.sim_time_seconds < end_s + 30.0
        ]
        assert recovery_ticks
        assert all(
            tick.p_imbalance_mw == pytest.approx(0.0, abs=1e-6)
            for tick in recovery_ticks
        )
...
def test_sj1_cluster_admission_is_independent_and_payload_keeps_identity() -> None:
    spec = ScenarioSpec.model_validate_json(_SCENARIO_PATH.read_text())
    # This test isolates independent cluster capacity accounting from the
    # scenario's time-shaped demand profile.  The production scenario is
    # intentionally at 120% during its first ten minutes.
    spec_data = spec.model_dump(mode="json")
    spec_data["gpu_load_profile"] = []
    ctx = build_run_context_from_spec(
...
def test_grid_import_is_physically_capped_and_d4_remains_balanced() -> None:
    ...
    # Shortfall is reported through p_imbalance_mw. p_unserved_mw remains
    # explicit shed only and is zero until protection actually sheds load.
    assert tick.d4_balance_defect_mw == pytest.approx(
        tick.frequency_forcing_mw,
        abs=1e-9,
    )
    assert tick.p_unserved_mw == pytest.approx(0.0, abs=1e-9)
    assert tick.p_imbalance_mw == pytest.approx(
        tick.frequency_forcing_mw,
        abs=1e-9,
    )
