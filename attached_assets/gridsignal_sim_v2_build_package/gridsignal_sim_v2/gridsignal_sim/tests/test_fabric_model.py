...
def load(name: str) -> Scenario:
    return Scenario.load(SCN / f"{name}.json", config_dir=CFG)


@pytest.fixture(scope="module")
def baseline_run():
    return run(load("S1_baseline_training"))
...
def test_tc81_checkpoint_produces_the_discriminating_signature():
    ...
    r = run(load("S2_checkpoint_hotspot"))
    ck = [
        (t, s)
...
def test_tc82_job_end_withholds_corroboration():
    ...
    r = run(load("S3_job_end_withholds"))
    je = [
        (t, s)
        for t, s, p in zip(r.ticks, r.samples, r.phases)
        if p == "job_end"
...
def test_tc82b_weight_load_read_does_not_corroborate():
    ...
    r = run(load("S1_baseline_training"))
    wl = [
        t
...
def test_tc84_control_path_breaches_nfr2_without_compute_loss():
    ...
    r = run(load("S4_control_path_nfr2_breach"))
...
@pytest.mark.parametrize(
    "name",
    [
        "S1_baseline_training",
        "S2_checkpoint_hotspot",
        "S3_job_end_withholds",
        "S4_control_path_nfr2_breach",
        "S5_gray_failure",
        "S6_baseline_tier_degradation",
        "S7_slow_checkpoint",
        "S8_transceiver_degrade",
    ],
)
