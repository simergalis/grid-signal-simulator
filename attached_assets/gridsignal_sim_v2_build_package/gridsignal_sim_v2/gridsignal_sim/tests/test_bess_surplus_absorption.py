...
def _make_rundown_state(
    *,
    turbine_count: int = 2,
    bess_soc: float = 0.0,
    bess_rated_mw: float = 20.0,
    bess_tau_s: float = 0.0,
    min_run_enabled: bool = False,
    unload_tail_s: float = 60.0,
) -> SimulationState:
    """Build an over-generating MSL fleet for BESS-first rundown tests."""
    site = SiteConfig(
        site_id="test-bess-rundown",
        pue_base=1.0, alpha_max=0.0, tau_seconds=20.0,
        dt_thermal_seconds=90.0, uncalibrated=False,
        workload_signal_stale_s=30.0,
        island_mode=IslandMode.ISLANDED,
        inertia_constant_s=4.0, frequency_nominal_hz=50.0,
    ...
    for index in range(turbine_count):
        turbine = TurbineModule(TurbineConfig(
            ...
            t_min_run_s=60.0,
            unload_tail_s=unload_tail_s,
        ))
        turbine.state = TurbineState.SYNCHRONISED
        turbine._current_output_mw = 10.0
        turbine._run_start_s = 0.0
        turbines.append(turbine)

    state = SimulationState(
        run_id="test-bess-rundown",
        site=site,
        gpu_modules=[GPUModule(
            asset_id="gpu-0", site=site, hardware_library=_HW, ramp_seconds=1.0,
        )],
        turbines=turbines,
        bess_units=[_make_bess(
            rated_mw=bess_rated_mw,
            usable_mwh=8.0,
            soc_fraction=bess_soc,
            tau_s=bess_tau_s,
        )],
        solar_arrays=[],
        cooling=CoolingModule(asset_id="cooling-0", site=site),
    )
    # Keep tests focused on the BESS gate rather than the configured 300 s
    # confirmation period; production continues to use its catalogue value.
    state._decommit_cond.threshold_s = 0.0
    return state
...
class TestBessFirstTurbineRundown:

    def test_rundown_waits_while_bess_can_absorb_all_surplus(self):
        """A charging BESS must prevent an otherwise-eligible decommit."""
        state = _make_rundown_state(bess_soc=0.0, bess_rated_mw=20.0)
        ...
        assert all(t.state == TurbineState.SYNCHRONISED for t in state.turbines)
        assert state._decommit_cond.sustained_s == 0.0

    def test_charge_saturated_bess_starts_controlled_rundown(self):
        """Charge-power saturation, not only a full SoC, enables a stop."""
        state = _make_rundown_state(bess_soc=0.0, bess_rated_mw=2.0)

        tick = _run_tick(state, sim_time=0.0, dt=5.0)

        assert tick.bess_output_mw == pytest.approx(-2.0, abs=1e-6)
        assert tick.commitment_action == "decommit"
        assert "BESS charge-saturated" in tick.commitment_reason
        assert state.turbines[-1].state == TurbineState.UNLOADING

    def test_inverter_response_lag_does_not_mimic_charge_saturation(self):
        """A non-full BESS keeps priority while its inverter response catches up."""
        state = _make_rundown_state(
            bess_soc=0.0,
            bess_rated_mw=20.0,
            bess_tau_s=10.0,
        )
        ...
        assert tick.bess_output_mw < 0.0
        assert -tick.bess_output_mw < 8.0  # lagged, but still physically capable
        assert tick.commitment_action == "hold"
        assert "charge-saturated" not in tick.commitment_reason
        assert all(t.state == TurbineState.SYNCHRONISED for t in state.turbines)

    def test_full_bess_starts_rundown_and_exposes_reason_in_tick_payload(self):
        """Full BESS surplus selects the last committed unit for unloading."""
        state = _make_rundown_state(bess_soc=1.0)

        tick = _run_tick(state, sim_time=5.0, dt=5.0)

        assert tick.bess_output_mw == pytest.approx(0.0, abs=1e-6)
        assert tick.commitment_action == "decommit"
        assert tick.commitment_target_unit_id == "gt-rundown-2"
        assert "BESS full" in tick.commitment_reason
        assert "turbine surplus remains after BESS absorbed" in tick.commitment_reason
        assert tick.fleet_utilisation == pytest.approx(0.0)
        assert tick.reserve_floor_mw == pytest.approx(10.0)
        assert state.turbines[-1].state == TurbineState.UNLOADING

    def test_minimum_run_deferral_keeps_bess_full_rundown_evidence(self):
        """A minimum-run guard blocks the stop without hiding its cause."""
        state = _make_rundown_state(bess_soc=1.0, min_run_enabled=True)

        tick = _run_tick(state, sim_time=5.0, dt=5.0)

        assert tick.commitment_action == "hold"
        assert "BESS full" in tick.commitment_reason
        assert tick.commitment_blocked_by is not None
        assert "r5_min_run_not_elapsed" in tick.commitment_blocked_by
        assert all(t.state == TurbineState.SYNCHRONISED for t in state.turbines)

    def test_next_rundown_waits_for_active_unload_sequence(self):
        """A second shutdown remains deferred until the first unit has settled."""
        state = _make_rundown_state(
            turbine_count=3,
            bess_soc=1.0,
            unload_tail_s=60.0,
        ...
        first = _run_tick(state, sim_time=0.0, dt=5.0)
        second = _run_tick(state, sim_time=5.0, dt=5.0)

        assert first.commitment_action == "decommit"
        assert sum(t.state == TurbineState.UNLOADING for t in state.turbines) == 1
        assert second.commitment_action == "hold"
        assert second.commitment_blocked_by == (
            "another turbine is still in the controlled unloading sequence"
        )
        assert "BESS full" in second.commitment_reason

    def test_active_unload_uses_same_reserve_fleet_for_decision_and_telemetry(self):
        """An unloading turbine is on-bus but must not inflate reserve capacity."""
        state = _make_rundown_state(
            turbine_count=3,
            bess_soc=1.0,
            unload_tail_s=60.0,
        )
        state.compute_floor_mw = 4.0
        ...
        # Two 10 MW synchronised units supply the reserve calculation; the
        # third is on-bus only to finish unloading at its 4 MW stable floor.
        assert second.committed_rated_mw == pytest.approx(20.0)
        assert second.fleet_utilisation == pytest.approx(4.0 / 20.0)
        assert second.reserve_floor_mw == pytest.approx(14.0)
        assert "U=0.200" in second.commitment_reason
