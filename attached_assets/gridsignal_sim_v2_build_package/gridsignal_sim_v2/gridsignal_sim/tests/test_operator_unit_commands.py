...
def _force_turbines_offline(ctx) -> None:
    """Make the command fixture explicitly represent an offline fleet.

    Factory-built production scenarios pre-synchronise ordinary turbines so
    they are available at the first simulation tick.  These command tests
    exercise the OFFLINE validation path and must opt into that state
    explicitly rather than relying on the factory's production default.
    """
    ...
    while ctx._operator_commands:
        ...
        for turb in ctx.sim_state.turbines:
            if turb.config.asset_id == uid:
                if action == "trip" and turb.config.hot_standby and turb.state == TurbineState.OFFLINE:
                    turb.config.hot_standby = False
...
def test_tc_203_2_start_command_enters_starting_and_produces_zero():
    ...
    Setup:
      Both units explicitly forced to OFFLINE for this command fixture.
    Expect:
...
def test_tc_203_5c3_trip_hot_standby_releases_and_queues_sync():
    """TRIP on OFFLINE hot standby releases it into synchronization."""
    from core.asset_modules import TurbineState
    ...
    spec = {
        **_SPEC,
        "turbine_units": [
            {"asset_id": "gt-0", "rated_mw": 10.0, "r_asset_mw_per_s": 0.5,
             "hot_standby": True, "hot_start_s": 0.0},
        ],
    }
    ctx = build_run_context_from_spec("tc-203-5c3", spec)
    manager = _make_manager_with_ctx(ctx)

    code, detail = manager.validate_and_enqueue_unit_command(ctx.run_id, "gt-0", "trip")
    assert code == manager.UNIT_CMD_OK, f"unexpected code {code!r}: {detail!r}"
    assert ctx._operator_commands == [{"unit_id": "gt-0", "action": "trip"}]

    _drain_operator_commands(ctx)
    gt0 = ctx.sim_state.turbines[0]
    assert gt0.config.hot_standby is False
    assert gt0.state == TurbineState.STARTING
