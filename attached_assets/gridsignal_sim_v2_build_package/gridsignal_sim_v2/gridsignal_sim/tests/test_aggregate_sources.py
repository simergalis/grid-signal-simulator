...
Source status:
  ...
  FUEL CELL — fuel_cell_output_mw is now wired into the physics engine. ✓
               Local merit order: BESS → TURBINE → FUEL CELL.  Dispatched by
               evaluate_tick() when demand exceeds cheaper available capacity.
...
class TestFuelCellAggregate:
    ...
    def test_FC10_turbine_is_preferred_before_fuel_cell(self):
        """
        FC-AGG-10: after BESS is unavailable, ordinary load is supplied by the
        $55/MWh turbine before the $65/MWh fuel-cell array.  The fuel cell may
        bridge turbine ramp delivery, but must return to zero once the cheaper
        turbine has settled within its available capacity.
        """
        from core._plane_guard import _EVALUATE_TICK_PERMITTED
        from core.sim_clock import SimClock
        from core.simulation_core import evaluate_tick
        ...
        assert settled.turbine_output_mw > 2.0 + 1e-6, (
            "FC-AGG-10: the cheaper turbine was not loaded above its minimum "
            "stable output after BESS became unavailable."
        )
        assert settled.fuel_cell_output_mw <= 1e-6, (
            "FC-AGG-10: the $65/MWh fuel cell remained dispatched while the "
            "$55/MWh turbine still had available capacity."
        )
        assert abs(settled.p_generation_mw - settled.p_demand_mw) < 1e-6, (
            "FC-AGG-10: cost-based dispatch must still close the "
            "production/consumption balance."
        )

    def test_FC11_all_three_sources_follow_cost_order_at_capacity(self):
        """
        FC-AGG-11: one settled high-load tick demonstrates the complete local
        merit order.  The $38/MWh BESS and $55/MWh turbine must both reach their
        available ceilings before the $65/MWh fuel cell supplies the residual.
        """
        from core._plane_guard import _EVALUATE_TICK_PERMITTED
        from core.sim_clock import SimClock
        from core.simulation_core import evaluate_tick
        ...
        assert settled.bess_output_mw == pytest.approx(1.0, abs=1e-6), (
            "FC-AGG-11: the cheapest BESS did not reach its available ceiling "
            "before the higher-cost sources were dispatched."
        )
        assert settled.turbine_output_mw == pytest.approx(5.0, abs=1e-6), (
            "FC-AGG-11: the turbine did not reach its available ceiling before "
            "the higher-cost fuel cell was dispatched."
        )
        assert settled.fuel_cell_output_mw > 0.01, (
            "FC-AGG-11: fuel cell did not supply the residual after cheaper "
            "BESS and turbine capacity was exhausted."
        )
        assert settled.fuel_cell_output_mw == pytest.approx(
            settled.p_demand_mw
            - settled.bess_output_mw
            - settled.turbine_output_mw,
            abs=1e-6,
        )
        assert abs(settled.p_generation_mw - settled.p_demand_mw) < 1e-6
    ...
    def test_FC12_energy_limited_bess_does_not_skip_turbine(self):
        """
        FC-AGG-12: a nearly empty BESS may reserve only the energy it can deliver
        during the tick.  Once that energy is exhausted, available $55/MWh
        turbine headroom must serve the load before the $65/MWh fuel cell.
        """
        from core._plane_guard import _EVALUATE_TICK_PERMITTED
        from core.sim_clock import SimClock
        from core.simulation_core import evaluate_tick
        ...
        settled = ticks[-1]
        assert settled.bess_output_mw <= 1e-9
        ...
        assert settled.fuel_cell_output_mw <= 1e-9, (
            "FC-AGG-12: fuel cell dispatched after the BESS became energy "
            "limited even though cheaper turbine headroom remained."
        )
        assert abs(settled.p_generation_mw - settled.p_demand_mw) < 1e-6
...
class TestSwitchgearThreeSource:
    ...
    def test_SW11_bess_covers_load_before_fuel_cell_without_turbine(self):
        """
        SW-AGG-11: In grid-connected mode with no turbine, the $38/MWh BESS
        supplies the 20-node load before the $65/MWh fuel-cell array.  Grid does
        not need to import.
        ...
        Expected:
          ...
          • BESS output is positive
          • fuel_cell_output_mw remains 0 MW while BESS capacity is available
        ...
        # BESS is the cheapest configured source and should cover this load
        # without calling the higher-cost fuel cell.
        ...
        assert bess_dispatched_any, (
            ...
            "check marginal-cost dispatch wiring"
        )
        for i, tick in enumerate(ticks):
            assert tick.fuel_cell_output_mw <= 1e-9, (
                f"SW-AGG-11 tick {i}: fuel_cell_output_mw="
                f"{tick.fuel_cell_output_mw:.6f} — fuel cell should remain idle "
                "while cheaper BESS capacity covers demand."
            )
