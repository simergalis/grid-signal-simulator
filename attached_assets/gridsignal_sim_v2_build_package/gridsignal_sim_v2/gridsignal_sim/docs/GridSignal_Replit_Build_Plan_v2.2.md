...
| **PA-7** | **The WorkloadSignal contract spans two integration surfaces, and §10 names only one.** §6.2 already hedges correctly — *"if the scheduler/framework integration emits one"* — but TC-05 is titled *"Explicit **scheduler** checkpoint event"* and TC-51 states *"The **scheduler** event is authoritative,"* both describing an event no scheduler emits. Slurm, Kubernetes, and Ray have no visibility into when a training job writes a checkpoint; that comes from the framework (a PyTorch hook, DeepSpeed checkpointing, or a job wrapper). Split the contract explicitly: **scheduler/orchestrator** → `queued`, `starting`, `running`, `scale`, `job_end`, `cancelled`; **training framework** → `checkpoint_start`, `checkpoint_end`. Retitle TC-05 and correct TC-51's wording. This also explains why §6.2's shape heuristic exists — the explicit pair requires instrumentation that cannot be assumed, whereas lifecycle events come free from an API already being read |
| **PA-8** | **"Job scheduler" is imprecise as the umbrella term** (used 7 times; "scheduler" 60 times). It is exact for Slurm, whose own name is Slurm Workload Manager. Kubernetes is a container *orchestrator* — Jobs are one workload type among Deployments, StatefulSets, and DaemonSets, and AI batch work generally needs Kueue or Volcano layered on for queueing at all. Ray is a distributed execution framework. Consider **"workload orchestration"** or **"cluster scheduler"** (the systems-literature term covering Borg, Omega, Mesos, YARN, Kubernetes, and Slurm alike). Cosmetic against PA-7, but the two should be corrected together |
