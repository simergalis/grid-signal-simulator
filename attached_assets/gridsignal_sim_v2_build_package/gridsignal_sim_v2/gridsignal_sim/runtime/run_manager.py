...
def _tick_result_to_dict(tick: TickResult) -> dict:
    ...
    return {
        ...
        # p_imbalance_mw: generation − demand (positive = surplus, negative = deficit).
        # Phase 6 (evaluate_tick) will replace this with the served-load variant when wired;
        # until then we use p_generation_mw − p_demand_mw, which equals the Phase 6 definition
        # in the common (no-UFLS-shed) case and gives the frontend a real non-null value now.
        "p_imbalance_mw":        round(tick.p_generation_mw - tick.p_demand_mw, 4),
