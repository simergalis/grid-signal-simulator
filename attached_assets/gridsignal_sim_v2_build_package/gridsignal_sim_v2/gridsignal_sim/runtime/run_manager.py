...
from core.site_parameters import get as _sp_catalogue_get
...
def _tick_result_to_dict(tick: TickResult) -> dict:
        ...
        [
            {
                ...
                "capacity_unit":       j.capacity_unit,
                "gpus_per_unit":       j.gpus_per_unit,
            }
    ...
    _cluster_metrics_list = (
        [
            {
                "cluster_id":          cluster.cluster_id,
                "tenant_id":           cluster.tenant_id,
                "scheduler_type":      cluster.scheduler_type,
                "hardware_profile_id": cluster.hardware_profile_id,
                "capacity_unit":       cluster.capacity_unit,
                "gpus_per_unit":       cluster.gpus_per_unit,
                "max_units":           cluster.max_units,
                "scheduled_units":     cluster.scheduled_units,
                "admitted_units":      cluster.admitted_units,
                "gpu_capacity":        cluster.gpu_capacity,
                "rated_capacity_mw":   cluster.rated_capacity_mw,
                "utilization":         round(cluster.utilization, 4),
            }
            for cluster in _km.cluster_metrics
        ]
        if _km is not None else []
    )
    ...
    return {
        ...
        # Phase 6: p_unserved_mw is explicit UFLS/curtailment shed. p_served_mw
        # is demand less that shed; p_imbalance_mw carries the supply shortfall.
        ...
        # p_imbalance_mw: supply minus demand after explicit shed. Negative is a
        # real supply shortfall and must not be reconstructed in this transport layer.
        ...
        "diesel_n_active_target": tick.diesel_n_active_target,
        "diesel_n_standby_available": tick.diesel_n_standby_available,
        "diesel_n_out_of_service": tick.diesel_n_out_of_service,
        "diesel_fleet_state": tick.diesel_fleet_state,
        "diesel_activation_episode_id": tick.diesel_activation_episode_id,
        "diesel_time_in_state_s": round(tick.diesel_time_in_state_s, 4),
        "diesel_fuel_remaining_gal": round(tick.diesel_fuel_remaining_gal, 4),
        "diesel_fuel_runtime_hours_remaining": (
            round(tick.diesel_fuel_runtime_hours_remaining, 4)
            if tick.diesel_fuel_runtime_hours_remaining is not None else None
        ),
        # Deliberate Phase 4 scope boundary: the diesel start shortfall is
        # surfaced in the Diesel Generation panel, not promoted to AlertDock.
        "diesel_insufficient_start_shortfall_mw": (
            round(tick.diesel_insufficient_start_shortfall_mw, 4)
            if tick.diesel_insufficient_start_shortfall_mw is not None else None
        ),
        ...
        "contingency_release_alert": tick.contingency_release_alert,
        "contingency_release_turbine_id": tick.contingency_release_turbine_id,
        "contingency_release_coverage_state": tick.contingency_release_coverage_state,
        "contingency_release_shed_mw": round(tick.contingency_release_shed_mw, 4),
        "contingency_release_sim_time_s": tick.contingency_release_sim_time_s,
        ...
        # The frontend AlertDock latches this on rising edge until the operator acknowledges.
        "balance_alert": abs(tick.p_generation_mw - tick.p_demand_mw) > 0.5,
        ...
        "kube_metrics": (
            {
                ...
                "cluster_metrics":    _cluster_metrics_list,
                "total_gpu_capacity": _km.total_gpu_capacity,
                "total_capacity_mw":  round(_km.total_capacity_mw, 4),
            }
            if _km is not None
            else None
        ),
        ...
        # Time-indexed future workload forecast for rolling planning.
        "planning_forecast_mw": [
            {
                "minutes_from_now": point["minutes_from_now"],
                "forecast_mw": round(point["forecast_mw"], 4),
            }
            for point in tick.planning_forecast_mw
        ],
        ...
        # Authoritative physical ledger result attached by evaluate_tick().
        # Keep the complete result together so monitoring, persistence, and
        # completed-run gating all consume the same computed object.
        "physical_balance": (
            tick.physical_balance.to_dict()
            if tick.physical_balance is not None else None
        ),
        ...
        # Contracted capacity gate (Task #510)
        "capacity_gate_deferred_count": tick.capacity_gate_deferred_count,
        "capacity_gate_headroom_mw":    round(tick.capacity_gate_headroom_mw, 4),
        ...
        "commitment_block": {
            ...
            "utilisation_threshold": (
                round(tick.commit_utilisation_threshold, 3)
                if tick.commit_utilisation_threshold is not None else None
            ),
...
def _attach_reserve_alert_proposal(ctx: "RunContext", tick: TickResult) -> TickResult:
    """Associate a reserve alert with one deterministic Generation proposal.

    This is advisory-only enrichment after the physics result exists.  It never
    changes the dispatch result; it only records the proposal in the existing
    AgentRegistry/AdvisoryGate and carries its ID to the operator UI.
    """
    if not tick.insufficient_reserve_alert:
        return tick
    existing_id = ctx.reserve_alert_proposal_ids.get(tick.tick_index)
    if existing_id is not None:
        return _dc_replace(tick, insufficient_reserve_proposal_id=existing_id)
    ...
    proposal = make_proposal(
        ...
        estimated_impact_mw=max(0.1, min(20.0, float(tick.peak_shortfall_mw))),
        confidence=0.55,
        reasoning=(
            "Insufficient reserve at dispatch staging: recommend turbine ramp "
            f"capacity for {tick.peak_shortfall_mw:.3f} MW shortfall over "
            f"{tick.turbine_estimated_time_to_close_s or 0.0:.1f} s."
        ),
        created_at_sim_time=tick.sim_time_seconds,
    )
    ...
    if not gate.validate(proposal):
        return tick
    ctx.reserve_alert_proposal_ids[tick.tick_index] = proposal.proposal_id
    return _dc_replace(tick, insufficient_reserve_proposal_id=proposal.proposal_id)
...
class RunContext:
    ...
    reserve_alert_proposal_ids: dict[int, str] = field(default_factory=dict)
    ...
    # Live scheduler events accepted by the ingestion boundary.  This is kept
    # separate from the scenario's static event list so repeated poll snapshots
    # can be acknowledged without applying the same signal twice.
    _ingested_event_ids: set[str] = field(default_factory=set, repr=False)
    # Latest accepted Slurm state per job.  Polling can arrive out of order;
    # never let a delayed RUNNING snapshot resurrect a terminal job.
    _slurm_job_states: dict[str, WorkloadEventType] = field(default_factory=dict, repr=False)
    ...
    def _planning_forecast(self, current_forecast_mw: float) -> tuple[dict[str, float], ...]:
        """Project scripted future workload additions at 15-minute samples.

        The live forecast is intentionally current-state-only.  Rolling
        planning also needs the known future scripted STARTING events, but
        must not run a second physics simulation.  Add each future event's
        full-TDP contribution to the current forecast and leave cooling and
        firm-capacity handling to the dashboard's existing projection logic.
        """
        future_events = [
            event for event in self.events
            if event.timestamp > self.sim_time
            and getattr(event.event_type, "value", str(event.event_type))
            in {"starting", "job_end", "cancelled"}
        ]
        if not future_events:
            return ()

        libraries = [
            module.hardware_library
            for module in self.sim_state.gpu_modules
            if getattr(module, "hardware_library", None)
        ]
        active_job_mw: dict[str, float] = {}
        for module in self.sim_state.gpu_modules:
            for job_id, nodes in module._node_counts.items():
                profile = module.hardware_library.get(
                    module._job_profiles.get(job_id, "")
                )
                rated_kw = profile.rated_kw if profile is not None else 12.0
                active_job_mw[job_id] = (
                    active_job_mw.get(job_id, 0.0)
                    + nodes * rated_kw * self.sim_state.site.pue_base / 1000.0
                )
        points: list[dict[str, float]] = []
        for minutes in range(15, 241, 15):
            horizon_s = self.sim_time + minutes * 60.0
            forecast_mw = current_forecast_mw
            future_job_mw: dict[str, float] = dict(active_job_mw)
            for event in future_events:
                if event.timestamp > horizon_s:
                    break
                event_type = getattr(event.event_type, "value", str(event.event_type))
                if event_type == "starting":
                    profile = next(
                        (library.get(event.hardware_profile_id) for library in libraries
                         if library.get(event.hardware_profile_id) is not None),
                        None,
                    )
                    rated_kw = profile.rated_kw if profile is not None else 12.0
                    contribution = (
                        event.node_count * rated_kw * self.sim_state.site.pue_base / 1000.0
                    )
                    future_job_mw[event.job_id] = contribution
                    forecast_mw += contribution
                elif event_type in {"job_end", "cancelled"}:
                    forecast_mw -= future_job_mw.pop(event.job_id, 0.0)
            points.append({
                "minutes_from_now": float(minutes),
...
class CompletedRun:
    ...
    # Authority of the physical ledger. Grid-tied runs remain provisional until
    # a PCC value independent of the dispatch residual is available.
    balance_independent: Optional[bool] = None
    balance_verification_mode: Optional[str] = None
...
class RunManager:
    ...
    def create_advisory_registry(self, registry_id: str) -> Any:
        """Create a standalone advisory registry for analytical proposals.

        Capacity Outlook is not a simulation and therefore must not fabricate a
        RunContext or start a physics task just to submit a human-review
        proposal.  The proposal lifecycle itself is still owned by the normal
        AgentRegistry/AdvisoryGate path, and the registry is addressable through
        the existing proposals endpoint using this analytical registry ID.
        """
        from advisory.agent_registry import AgentRegistry
    ...
    async def start_run(self, ctx: RunContext) -> str:
        ...
        ctx._ingested_event_ids.update(event.event_id for event in ctx.events)
        self._contexts[ctx.run_id] = ctx
        task = asyncio.create_task(self._drive(ctx), name=f"run-{ctx.run_id}")
        self._tasks[ctx.run_id] = task
        return ctx.run_id

    def ingest_workload_signal(
        self, run_id: str, signal: WorkloadSignal
    ) -> tuple[str, str]:
        """Queue one external WorkloadSignal for the next simulation tick.

        The API layer performs wire-format translation; this method owns the
        active-run boundary and keeps event insertion safe relative to the
        sorted scenario event stream.  Returns ``("accepted", "")`` for a new
        event, ``("duplicate", "")`` for an idempotent poll retry, or an
        explicit error code/detail for callers.
        """
        ctx = self._contexts.get(run_id)
        if ctx is None or ctx.is_complete():
            return self.UNIT_CMD_RUN_404, f"Run {run_id!r} not found or not active."
        if signal.site_id != ctx.sim_state.site.site_id:
            return "site_mismatch", (
                f"Signal site_id {signal.site_id!r} does not match "
                f"run site_id {ctx.sim_state.site.site_id!r}."
            )
        if signal.event_id in ctx._ingested_event_ids:
            # Preserve the original event timestamp in the acknowledgement so
            # a retry receives the same WorkloadSignal representation even if
            # the simulation advanced between poll attempts.
            for existing in ctx.events:
                if existing.event_id == signal.event_id:
                    signal.timestamp = existing.timestamp
                    break
            return "duplicate", ""

        if signal.scheduler_type == "slurm":
            _slurm_state_rank = {
                WorkloadEventType.QUEUED: 0,
                WorkloadEventType.RUNNING: 1,
                WorkloadEventType.JOB_END: 2,
                WorkloadEventType.CANCELLED: 2,
            }
            previous = ctx._slurm_job_states.get(signal.job_id)
            previous_rank = _slurm_state_rank.get(previous, -1)
            incoming_rank = _slurm_state_rank.get(signal.event_type, -1)
            if previous_rank > incoming_rank or (
                previous_rank == 2 and incoming_rank == 2
            ):
                return "stale_event", (
                    f"Slurm job {signal.job_id!r} is already in terminal state "
                    f"{previous.value if previous is not None else 'unknown'!r}; "
                    f"refusing delayed {signal.event_type.value!r} snapshot."
                )

        # External events are timestamped by the API at the current simulation
        # time.  Guard against an out-of-order caller so a newly queued event
        # cannot land behind the already-consumed event cursor.
        signal.timestamp = max(signal.timestamp, ctx.sim_time)
        ctx.events.append(signal)
        ctx.events.sort(key=lambda event: event.timestamp)
        ctx._ingested_event_ids.add(signal.event_id)
        if signal.scheduler_type == "slurm":
            ctx._slurm_job_states[signal.job_id] = signal.event_type
        return "accepted", ""
    ...
    def validate_and_enqueue_unit_command(
        ...
        _standby_trip = (
            action == "trip"
            and turbine.state == _TS.OFFLINE
            and turbine.config.hot_standby
        )
        if action == "trip" and turbine.state not in _ON_BUS and not _standby_trip:
            return self.UNIT_CMD_BAD_STATE, (
                f"Unit {unit_id!r} is in state {turbine.state.value!r} — "
                "trip is only valid for on-bus units "
                "(synchronised / unloading), or an OFFLINE hot-standby unit "
                "being released to synchronize."
            )
        ...
        # The normal "start" action remains unavailable for hot-standby units.
        # Their explicit operator release action is "trip" so the command is
        # visible as the requested TRIP operation in the fleet panel.
        ...
        try:
            ...
            while not ctx.is_complete():
                ...
                while ctx._operator_commands:
                    ...
                    for _turb in ctx.sim_state.turbines:
                        if _turb.config.asset_id == _cmd_uid:
                            if _cmd_action == "trip" and _turb.config.hot_standby and _turb.state == _TurbineState.OFFLINE:
                                # Operator TRIP on standby means release the
                                # commissioned unit into the normal sync path.
                                # Clear the dispatch-only flag before calling
                                # command_start(), whose hot-standby guard
                                # otherwise intentionally returns.
                                _turb.config.hot_standby = False
                                _turb.command_start(ctx.sim_time)
                                logger.info(
                                    "operator command: TRIP standby turbine %r "
                                    "→ synchronization at sim_time=%.1f (run=%s)",
                                    _cmd_uid, ctx.sim_time, ctx.run_id,
                                )
                            elif _cmd_action == "trip":
                ...
                tick_result  = _dc_replace(
                    ...
                    # ── Contracted capacity gate (Task #510) ─────────────────
                    # capacity_gate_deferred_count: jobs deferred by MW cap gate
                    #   this tick (summed from kube_metrics across all agents).
                    # capacity_gate_headroom_mw: design_peak − compute demand.
                    #   Positive = headroom remaining; negative = over cap.
                    capacity_gate_deferred_count=(
                        tick_result.kube_metrics.cap_gate_deferred_count
                        if tick_result.kube_metrics is not None else 0
                    ),
                    capacity_gate_headroom_mw=(
                        round(ctx._design_peak_load_mw - tick_result.p_compute_demand_mw, 4)
                        if ctx._design_peak_load_mw > 0.0 else 0.0
                    ),
                )
                ...
                # Associate the immutable physics alert with the existing
                # Generation advisory proposal before persistence/broadcast.
                # This enrichment does not alter any physics or dispatch field.
                tick_result = _attach_reserve_alert_proposal(ctx, tick_result)
                ...
                # ── Runtime error log: physical balance violation ─────────
                # This consumes the invariant attached to TickResult rather
                # than reconstructing a second balance calculation here.
                if (
                    tick_result.physical_balance is not None
                    and not tick_result.physical_balance.passed
                ):
                    import json as _json_err
                    _balance = tick_result.physical_balance
                    await ctx.sink.append_error(
                        error_kind="balance_violation",
                        run_id=ctx.run_id,
                        message=(
                            f"physical balance invariant does not close: "
                            f"{_balance.defect_mw:+.4g} MW "
                            f"at tick {tick_result.tick_index} "
                            f"(sim_time={tick_result.sim_time_seconds:.1f}s)"
                        ),
                        detail=_json_err.dumps(_balance.to_dict()),
            ...
            if not _skip_verdict:
                ...
                try:
                    ...
                    _pue_entry = _sp_catalogue_get("pue_base")
                    _pue_bounds = (
                        (float(_pue_entry.min), float(_pue_entry.max))
                        if _pue_entry.min is not None and _pue_entry.max is not None
                        else None
                    )
                    verdict_result = evaluate_verdict(
                        ...
                        pue_base=ctx.sim_state.site.pue_base,
                        pue_base_bounds=_pue_bounds,
                    )
                ...
                # Collect the already-evaluated physical invariant from every
                # tick and call gate_run so the result screen can surface whether
                # the identity closed.
                ...
                _balance_independent: Optional[bool] = None
                _balance_verification_mode: Optional[str] = None
                try:
                    _bal_tol = _sp_catalogue_value("balance_defect_tolerance_mw")
                    if tick_dicts and _bal_tol is not None:
                        _balances = [td.get("physical_balance") for td in tick_dicts]
                        if not all(isinstance(b, dict) for b in _balances):
                            raise ValueError(
                                "physical balance invariant missing from one or more ticks"
                            )
                        _independence = {bool(b["independent"]) for b in _balances}
                        _verification_modes = {
                            str(b["verification_mode"]) for b in _balances
                        }
                        if len(_independence) != 1 or len(_verification_modes) != 1:
                            raise ValueError(
                                "physical balance verification authority changed within run"
                            )
                        _defects = [float(b["defect_mw"]) for b in _balances]
                        _balance_gate = _pb_gate_run(_defects, tolerance_mw=_bal_tol)
                        _balance_independent = _independence.pop()
                        _balance_verification_mode = _verification_modes.pop()
                except Exception:
                ...
                # preserves the "EDL not active" vs "zero cost" distinction.
                ...
                self._completed[ctx.run_id] = CompletedRun(
                    ...
                    balance_independent=_balance_independent,
                    balance_verification_mode=_balance_verification_mode,
