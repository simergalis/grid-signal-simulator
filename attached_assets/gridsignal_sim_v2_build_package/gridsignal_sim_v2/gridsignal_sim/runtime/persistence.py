...
def _tick_to_json(tick: TickResult) -> str:
    ...
    try:
        ...
        # BalanceResult exposes derived pass/magnitude/breakdown properties;
        # persist that same authoritative representation rather than making
        # consumers reconstruct it from the dataclass internals.
        if tick.physical_balance is not None:
            payload["physical_balance"] = tick.physical_balance.to_dict()
...
class Site(Base):
    ...
    def effective_pue(self) -> float:
        """Derived from persisted source fields; intentionally not stored."""
...
class ReferenceForecastScenario(Base):
    """Durable identity for an imported historical/reference forecast dataset.

    This is intentionally independent from Scenario, whose primary key is
    run-scoped ``run_id``.  The dataset identifier is owned by this table and
    remains stable across imports and simulator runs.
    """

    __tablename__ = "reference_forecast_scenario"
...
class ReferenceForecastResolved(Base):
    """One resolved hourly workload row for a reference forecast dataset."""

    __tablename__ = "reference_forecast_resolved"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    dataset_id: Mapped[str] = mapped_column(
        String,
        ForeignKey("reference_forecast_scenario.dataset_id"),
        nullable=False,
        index=True,
    )
    day_of_year: Mapped[int] = mapped_column(Integer, nullable=False)
    hour_of_day: Mapped[int] = mapped_column(Integer, nullable=False)
    kubernetes_node_count: Mapped[int] = mapped_column(Integer, nullable=False)
    slurm_node_count: Mapped[int] = mapped_column(Integer, nullable=False)
    ray_rack_count: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "day_of_year",
            "hour_of_day",
            name="uq_reference_forecast_resolved_dataset_day_hour",
        ),
        CheckConstraint(
            "day_of_year BETWEEN 1 AND 366",
            name="ck_reference_forecast_resolved_day_of_year",
        ),
        CheckConstraint(
            "hour_of_day BETWEEN 0 AND 23",
            name="ck_reference_forecast_resolved_hour_of_day",
        ),
        CheckConstraint(
            "kubernetes_node_count >= 0 AND slurm_node_count >= 0 "
            "AND ray_rack_count >= 0",
            name="ck_reference_forecast_resolved_nonnegative_counts",
        ),
    )
...
class TraceImportReport(Base):
    """Durable Unified Periodic Trace Import report.

    The complete report is retained as JSON so the response contract remains
    owned by the import parser.  The indexed columns make the two primary
    lookups explicit without duplicating the report's nested row data.
    """

    __tablename__ = "trace_import_report"
    ...
    measurement_source: Mapped[str] = mapped_column(String, nullable=False)
    window_start: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    window_end: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    report_json: Mapped[str] = mapped_column(Text, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)


class TraceComparisonReport(Base):
    """Durable read-only comparison report, including all nested series."""

    __tablename__ = "trace_comparison_report"

    comparison_id: Mapped[str] = mapped_column(String, primary_key=True)
    import_id: Mapped[str] = mapped_column(String, nullable=False, index=True)
    site_id: Mapped[str] = mapped_column(String, nullable=False, index=True)
    window_start: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    window_end: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    report_json: Mapped[str] = mapped_column(Text, nullable=False)
...
class CapacityOutlookReport(Base):
    __tablename__ = "capacity_outlook_report"

    outlook_id: Mapped[str] = mapped_column(String, primary_key=True)
    import_id: Mapped[str] = mapped_column(String, nullable=False, index=True)
    site_id: Mapped[Optional[str]] = mapped_column(String, nullable=True, index=True)
    report_json: Mapped[str] = mapped_column(Text, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
class AlertDisposition(Base):
    """Durable operator disposition for one alert instance.

    A row is intentionally per instance even when a future bulk action creates
    several rows.  batch_reference links those rows without collapsing audit
    identity, freshness, or the proposed clustering reason.
    """
    ...
    alert_key: Mapped[str] = mapped_column(String(256), nullable=False, index=True)
    alert_type: Mapped[str] = mapped_column(String(64), nullable=False)
    tick_index: Mapped[int] = mapped_column(Integer, nullable=False)
    telemetry_fresh: Mapped[bool] = mapped_column(Boolean, nullable=False)
    disposition: Mapped[str] = mapped_column(String(32), nullable=False)
    reviewer_id: Mapped[str] = mapped_column(String(256), nullable=False)
    recorded_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    batch_reference: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    proposal_reason: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

    __table_args__ = (
        CheckConstraint(
            "disposition IN ('handled', 'not_handled')",
            name="ck_alert_disposition_value",
        ),
    )
...
class RuntimeErrorLog(Base):
    ...
    error during a run:
      ...
      'balance_violation' — the authoritative physical balance invariant failed
        on a tick.  The persisted detail is the exact invariant result,
        including its tolerance and term breakdown.
