...
_SEEDED: list[tuple[str, ScenarioSpec]] = [
    ...
    (
        ...
        ScenarioSpec(
            ...
            description=(
                "This scenario demonstrates how solar variability from partial cloud cover affects site operations during midday, when solar contribution is at its highest and the gap between solar output and full load is most sensitive to fluctuations. The simulation is anchored to solar noon at the SV1 (Silicon Valley) site so that clear-sky Mediterranean variability — passing cloud cover producing output swings between 65 and 98 percent of rated capacity — is visible on the solar panel regardless of when the demo actually runs. The operator sees generation mix shift as solar fades and recovers, with the turbine adjusting to compensate. This matters because midday is when solar optimism is most tempting and most dangerous: even a warm, dry site with mostly clear skies can lose a meaningful share of solar output to passing clouds during the peak compute window, and operators need a plan for it."
            ),
            workload_events=[_evt_start("job-big", _DEMO_NODES)],
            dt_lead_seconds=30.0,
            bess_units=[_bess("bess-0", rated_mw=18.0, usable_mwh=8.0, grid_forming=True)],
            ...
            solar_rated_mw=_SOLAR_DEMO,
            end_sim_time=300.0,
            solar_origin_utc_hour=20,   # UTC 20:00 = 12:00 PST — SV1 (Santa Clara / Silicon Valley) solar noon; same America/Los_Angeles tz as the prior San Diego anchor, so the hour value is unchanged
            gpu_load_profile=[],   # full GPU load throughout (no throttling)
        ),
    ),
    ...
    (
        ...
        ScenarioSpec(
            ...
            bess_units=[_bess("bess-0", rated_mw=18.0, usable_mwh=8.0, grid_forming=True)],
            turbine_units=[
                # turbine-0 starts online and synchronised to the bus.
                _turbine("turbine-0", rated_mw=25.0, r_mw_per_s=0.2),
                # turbine-1 — hot standby: pre-commissioned, fastest to commit.
                _turbine("turbine-1", rated_mw=25.0, r_mw_per_s=0.2, hot_standby=True, thermal_state="hot"),
                # turbine-2 — warm standby: off-bus, pre-warmed thermal state.
                _turbine("turbine-2", rated_mw=25.0, r_mw_per_s=0.2, hot_standby=True, thermal_state="warm"),
                # turbine-3 — cold standby: off-bus, full cold-start sequence required.
                _turbine("turbine-3", rated_mw=25.0, r_mw_per_s=0.2, hot_standby=True, thermal_state="cold"),
            ],
            solar_rated_mw=0.0,          # no solar — replaced by fuel cell array
            fuel_cell_enabled=True,
            # fuel_cell_rated_mw is the nameplate of ONE stack (see schemas.py).
            # scenario_factory.py multiplies by fuel_cell_stack_count to get the
            # fleet-total the physics engine dispatches: 5.0 MW/stack × 4 = 20 MW.
            fuel_cell_rated_mw=5.0,
            fuel_cell_stack_count=4,
            # Cascade commit: each turbine commits the next when it reaches 50% of
            # its own rated MW.  turbine-0 → turbine-1 at 12.5 MW; turbine-1 →
            # turbine-2 at 12.5 MW; turbine-2 → turbine-3 at 12.5 MW.
            ...
            gpu_load_profile=[
                # Zero-order-hold: fraction applies from its timestamp until the next.
                # Anchor at t=0 keeps full load before the first step; without it the
                # first entry's value would be used even before its own timestamp.
                (0.0,    1.00),   # t=0–12 min   : full load
                (720.0,  0.50),   # t=12–40 min  : 50 % (rate halved)
                (2400.0, 0.10),   # t=40 min–end : 10 % (decelerated)
            ],
    ...
    (
        ...
        ScenarioSpec(
            ...
            #   turbine-0 Hot  (fast-start frame: cold=300/warm=200/hot=100 s)
            #   turbine-1 Warm (mid-frame: cold=600/warm=400/hot=200 s)
            #   turbine-2 Warm (cold_start_s=900 — standard frame, pre-warmed)
            #   turbine-3 Cold (cold_start_s=900)
            #   turbine-4 Cold (cold_start_s=900)
            # All three start-duration fields supplied together to satisfy the
            # hot_start_s < warm_start_s < cold_start_s ordering invariant.
            turbine_units=[
                _turbine("turbine-0", rated_mw=15.0, r_mw_per_s=0.20,
                         p_min_stable_frac=0.40, thermal_state="hot",
                         cold_start_s=300.0, warm_start_s=200.0, hot_start_s=100.0),
                _turbine("turbine-1", rated_mw=15.0, r_mw_per_s=0.20,
                         p_min_stable_frac=0.40, thermal_state="warm",
                         cold_start_s=600.0, warm_start_s=400.0, hot_start_s=200.0),
    ...
    # ── test-fc-dispatch: minimal scenario to confirm FC fires when turb+BESS saturates ──
    # Turbine-0 is rated 5 MW; BESS is rated 1 MW.  Joint ceiling = 6 MW.
    # Kube demand grows past 6 MW by ~sim_t=70 s, leaving a residual the FC must fill.
    # headroom_threshold_mw=2.0 keeps job admissions open while total site capacity
    # (5 MW turbine + 20 MW FC) easily covers the load.
    (
        "test-fc-dispatch",
        ScenarioSpec(
            name="test-fc-dispatch",
            description=(
                "Minimal fuel-cell dispatch stress scenario. "
                "A single 5 MW turbine and 1 MW BESS (joint ceiling 6 MW) face a stochastic "
                "Kubernetes workload that quickly exceeds 6 MW, forcing the 20 MW Fuel Cell "
                "Module Array to cover the residual. "
                "Used only by test_fuel_cell_dispatch.py to verify the FC dispatch path."
            ),
            workload_events=[],
            dt_lead_seconds=0.0,
            bess_units=[_bess("bess-0", rated_mw=1.0, usable_mwh=0.5, grid_forming=True)],
            turbine_units=[
                # Single small turbine — rated 5 MW; starts SYNCHRONISED.
                # No cascade_commit_fraction so no additional turbines are committed.
                _turbine("turbine-0", rated_mw=5.0, r_mw_per_s=0.2),
            ],
            solar_rated_mw=0.0,
            fuel_cell_enabled=True,
            fuel_cell_rated_mw=20.0,   # per-stack rating; stack_count=1 → 20 MW fleet
            fuel_cell_stack_count=1,
            end_sim_time=600.0,        # 10 min; demand exceeds turbine+BESS by ~t=70 s
            gpu_load_profile=[],       # full load throughout — maximise FC trigger window
            kube_config=KubeConfigSpec(
                hardware_profile_id="enterprise_8gpu_air",
                max_nodes=1900,
                min_nodes=200,
                ...
                reorder_window_s=10.0,
                ntp_jitter_s=2.0,
                # Low threshold so the scheduler keeps admitting jobs once turbine
                # headroom is nearly zero — total site capacity is 25 MW (5 + 20),
                # so we allow admissions until only 2 MW of headroom remains.
                headroom_threshold_mw=2.0,
                rng_seed=42,           # same seed as kube-peak-overage → predictable arrival pattern
            ),
        ),
    ),
...
def _seed_fabric_scenarios(store: ScenarioStore) -> None:
    """Seed the eight fabric regression scenarios into the store.
    ...
    #                               capability_tier).
    # capability_tier is "degraded" only for the degraded-observability test.
    FABRIC_ENTRIES: list[tuple[str, str, float, str, str]] = [
        ("regression-test-healthy-training-baseline", "Regression test — Healthy training baseline", 60.0, "regression-test-healthy-training-baseline", "current"),
        ("regression-test-checkpoint-storage-hotspot", "Regression test — Checkpoint storage hotspot", 80.0, "regression-test-checkpoint-storage-hotspot", "current"),
        ("regression-test-clean-job-termination", "Regression test — Clean job termination", 60.0, "regression-test-clean-job-termination", "current"),
        ("regression-test-control-path-latency-isolation", "Regression test — Control-path latency isolation", 20.0, "regression-test-control-path-latency-isolation", "current"),
        ("regression-test-gray-link-failure", "Regression test — Gray link failure", 30.0, "regression-test-gray-link-failure", "current"),
        ("regression-test-degraded-fabric-observability", "Regression test — Degraded fabric observability", 30.0, "regression-test-degraded-fabric-observability", "degraded"),
        ("regression-test-slow-checkpoint", "Regression test — Slow checkpoint", 100.0, "regression-test-slow-checkpoint", "current"),
    ...
    for store_id, display_name, duration, fab_id, _cap_tier in FABRIC_ENTRIES:
        ...
        _jpath = _cfg_dir / f"{_legacy_filenames[fab_id]}.json"
        try:
...
def _seed_json_scenarios(store: ScenarioStore) -> None:
    ...
    _JSON_ENTRIES: list[tuple[str, str]] = [
        ...
        # 10-tenant full-ceiling stress — tight BESS + 3-turbine fleet
        ...
        # BESS + Fuel Cell + Grid only — 80%→120%→60% three-phase peak stress (60 min)
        ("demo-fc-bess-grid-peak", "demo-fc-bess-grid-peak.json"),
        # Kubernetes demand layer — GPU jobs scheduled without advance power notice;
        # battery and generators absorb each admission step reactively.
        ("scenario-kube-peak-overage", "scenario-kube-peak-overage.json"),
        # Four-hour planning replay with deterministic GPU peaks at 15m, 1h, and 3h.
        ("demo-rolling-planning-peaks", "demo-rolling-planning-peaks.json"),
        ...
        # Full-day SJ-2 customer replay — stored compressed to keep its
        # 100-event scheduler timeline as one deterministic catalog asset.
        ("scenario-equinix-sj-2-24h", "scenario-equinix-sj-2-24h.json.gz.b64"),
        # Islanded SJ-1 derivative with three 25 MW gas turbines held in
        ...
        # Addendum G-1 control: no initially-hot blocks, so the physical BESS
        # ceiling and residual unserved load remain directly observable.
        ("scenario-fc100-islanded-zero-hot-v2",
         "scenario-fc100-islanded-zero-hot-v2.json"),
    ]
    ...
    for store_id, filename in _JSON_ENTRIES:
        ...
        try:
            if filename.endswith(".json.gz.b64"):
                _raw = _gzip.decompress(
                    _base64.b64decode(_jpath.read_text())
                ).decode("utf-8")
            else:
                _raw = _jpath.read_text()
            _spec = ScenarioSpec.model_validate_json(_raw)
            if display_name := _DISPLAY_NAME_OVERRIDES.get(store_id):
                _spec = _spec.model_copy(update={"name": display_name})
            store._seed(store_id, _spec)
