...
async def _lifespan(application: FastAPI):
    ...
    # Ask Gridley audit records are intentionally process-local, matching the
    # current in-memory ScenarioStore boundary.  They never enter the physics
    # tick path and are replaced by durable audit persistence in a later step.
    application.state.gridley_audit = []
    # Pending AI remediation recommendations are short-lived, process-local
    # records. Execution revalidates against the latest simulator tick.
    application.state.ai_remediation = {}
    # Query backlog for Ask Gridley questions that do not map confidently to
    # the declared catalogue. This is intentionally outside the tick loop.
    application.state.gridley_unmatched_queries = []
